...
name: cardano-node
description: >
  Full-lifecycle guide for operating a haskell Cardano stake pool with cardano-node and
  ...
  whenever the user mentions a Cardano node, stake pool, block producer, relay,
  KES rotation, pool registration or retirement, cardano-tracer, Grafana for
  Cardano, SPO hardening, Mithril, Calidus, CNTools, gLiveView, CNCLI, Topology
  Updater, cardano-airgap, SPO governance, or hard-fork voting.
---

# Cardano Node & Stake Pool Operations

This skill is a current, full-parity reference for operating a Cardano stake pool on
mainnet or testnet. It tracks the official Cardano Developer Portal documentation under
`/docs/operate-a-stake-pool/` and bundles community best practices.

Raw markdown snapshots of every source page live under `references/sources/`. The
`scripts/sync_docs.sh` script re-fetches them and reports diffs so this skill can be
updated whenever the docs change.
...
To check whether the skill is current with the docs, run `scripts/sync_docs.sh`.
...
8. **If something feels out of date, run the sync script.** `scripts/sync_docs.sh`
   re-fetches every tracked page and reports diffs. The user can then refresh the
   skill content from the updated snapshots.
