...
- `python3 scripts/check-doc-status-headers.py` — central parity-doc status guard. Confirms `docs/PARITY_SUMMARY.md`, `docs/UPSTREAM_PARITY.md`, and `docs/COMPLETION_ROADMAP.md` agree on status-header date, round ceiling, parity tag, and test-baseline date; rejects a round ceiling behind the latest `docs/operational-runs/*round-*.md` record; cross-checks the parity tag and dashboard status counts against `docs/parity-matrix.json`; and keeps `docs/PARITY_DASHBOARD.md` aligned with the canonical status date. When editing the guard itself, run `python3 scripts/check-doc-status-headers.py --self-test` before the live scan. **CI gate** since R824.
