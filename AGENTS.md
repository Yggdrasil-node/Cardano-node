...
- `dev/specs/upstream-test-vectors` holds official test vectors from the `IntersectMBO` repositories.
...
- The reference parity target is **always the latest IntersectMBO/cardano-node release tag** (currently `11.0.1`). When upstream ships a new tag, bump in lockstep across `dev/reference/setup-reference.sh` (`CARDANO_NODE_VERSION`), `dev/test/check-parity-matrix.py` (`REFERENCE_TAG` + `ALLOWED_STATUS`), `docs/parity-matrix.json` (`reference.tag` + every `haskell_reference.path`), and prose mentions in this file. Pinning to a stale tag silently invalidates parity claims.
...
- `python3 dev/test/check-parity-matrix.py` — validates `docs/parity-matrix.json` schema + every `haskell_reference.path` and `rust_surface.path` exists on disk. Required when matrix entries, status, or paths change. **CI gate** since R303.
- `python3 dev/test/check-strict-mirror.py` — strict 1:1 file-mirror drift-guard. Walks production `.rs` files and flags any new file lacking either an upstream `.hs` mirror (by snake_case-of-PascalCase basename match) or a `## Naming parity` docstring stanza. Reads `docs/strict-mirror-audit.tsv` as the allowlist. Warn-only since R275 (`continue-on-error: true` in CI); flips to fail-build at R288.
- `python3 dev/test/check-type-parity.py` — exact-text type/name parity guard. Validates `docs/type-parity-audit.tsv` rows by checking that each declared Rust identifier/config key and its upstream Haskell counterpart both exist as identifier-boundary tokens in the expected mirror files, rejects duplicate mappings, rejects absolute or root-escaping manifest paths, requires Rust `.rs` paths under `crates/` plus upstream `.hs` paths under `.reference-haskell-cardano-node/`, requires lower-kebab-case scope names, requires single-token `rust_text` / `upstream_text` fields, requires explanatory sentence notes, and enforces deterministic row order. Required when adding, renaming, or completing a public parity surface listed in that manifest; run `python3 dev/test/check-type-parity.py --self-test` when editing the guard itself.
- `python3 dev/test/check-stale-placement.py` — post-reorganization path and status guard. Fails if current code, CI, generated navigation, commands, resolved Cargo metadata, current operational-run notes, `[Unreleased]` changelog entries, living docs, or exact filesystem paths point back at the legacy node-local crate, nested `crates/node/*/{configuration,scripts}` operator-artifact directories, root/yggdrasil-node shorthand metadata, tests, configuration, script, tool-crate, or retired AI-harness placements. It also rejects stale current-status claims from the cleanup arc, including obsolete node-local LSQ wording, old cardano-cli subcommand counts, three-command subset wording or active-migration wording, old parity-summary/proof/upstream verification baselines, old README/docs-site test baselines, stale pre-R258 BlockFetch default-flip wording, old cardano-cli gate wording for tx-generator/cardano-testnet, stale cardano-submit-api structured-decoder/R345-R346 evidence wording, stale kes-agent/kes-agent-control early-mini-arc current-status wording, stale root-manifest sister-tool labels, stale dmq-node pre-R816 current-status wording, stale cardano-testnet pre-R823/current Command-payload, process-handle type, Process/Cli/Keys gap wording, Transaction sign/submit/txid gap wording, and DRep pure-builder gap wording, and the closed workspace-member gap. It requires the accepted replacement placements to exist: `crates/node/cardano-node/`, the canonical root `configuration/` operator bundles, and dev operator/reference scripts; every tracked `dev/{scripts,evidence,reference}/*.sh` helper must remain executable in the Git index; release/repro workflows, Docker packaging, and the release installer must stage/copy/install root `configuration/` and `dev/` from their accepted locations. It bucket-checks Cargo metadata so the shipped `yggdrasil-node` package stays at `crates/node/cardano-node/`, node support packages stay under `crates/node/`, and sister-tool packages stay under `crates/tools/`. It also fails if the vendored Haskell reference snapshot contains nested `.git` metadata, is not ignored by Git, is declared/tracked as a Git submodule, or has any regular file in the Git index. Tagged changelog history, old operational-run records, and run logs are excluded. When editing the guard, run `python3 dev/test/check-stale-placement.py --self-test` as well.
- `python3 dev/test/check-doc-status-headers.py` — central parity-doc status guard. Confirms `docs/PARITY_SUMMARY.md`, `docs/UPSTREAM_PARITY.md`, and `docs/COMPLETION_ROADMAP.md` agree on status-header date, round ceiling, parity tag, and test-baseline date; rejects a round ceiling behind the latest `docs/operational-runs/*round-*.md` record; cross-checks the parity tag and dashboard status counts against `docs/parity-matrix.json`; and keeps `docs/PARITY_DASHBOARD.md` aligned with the canonical status date. When editing the guard itself, run `python3 dev/test/check-doc-status-headers.py --self-test` before the live scan. **CI gate** since R824.
- `python3 dev/test/check-fixture-manifest.py` — cross-checks the `cardano-base` SHA pin across `crates/node/config/src/upstream_pins.rs::UPSTREAM_CARDANO_BASE_COMMIT`, `dev/specs/upstream-test-vectors/cardano-base/<SHA>/`, `docs/SPECS.md`, and `docs/UPSTREAM_PARITY.md`; verifies every required upstream-vendored fixture corpus is present + non-empty. **CI gate** since R303.
- `python3 dev/test/check-reference-artifacts.py` — validates the vendored Haskell `.reference-haskell-cardano-node/install/` tree: required binaries are present + executable, every per-network share dir carries the canonical operator-config bundle, and `cardano-node --version` reports the policy tag from `docs/parity-matrix.json::reference.tag`. Linux/WSL local/operator gate (CI does not carry the 1.3 GB install tree).
- `python3 dev/test/audit-strict-mirror.py` — rebuilds `docs/strict-mirror-audit.tsv` after Phase B graduates rows. Required when the audit-table verdict for a Rust file changes (e.g., `git mv` rename, new `## Naming parity` block, or a new file lands).
- `python3 dev/test/filetree.py check` — flags stale `dev/filetree/manifest.json` description entries. Required when filename-mirror restructures (R271-style) move tracked files.
- `bash dev/reference/setup-reference.sh [--force]` — refreshes `.reference-haskell-cardano-node/` to the policy IntersectMBO tag. Required when the tag bumps or the local reference snapshot drifts.
...
not exist. The CI counterpart is `python3 dev/test/check-strict-mirror.py`; use
the policy in this file and the local upstream reference tree as the
authoring-time guidance. The
...
- [`docs/type-parity-audit.tsv`](docs/type-parity-audit.tsv) — exact-text identifier/config-key mapping for public parity surfaces that need a stronger type/name audit than filename parity alone. Validated by `dev/test/check-type-parity.py`.
- [`docs/upstream-haskell-files.txt`](docs/upstream-haskell-files.txt) — flat-file index of every `.hs` under the vendored upstream tree, rebuilt by `dev/reference/setup-reference.sh`.
- [`dev/test/filetree.py`](dev/test/filetree.py) — checked-in filetree manifest maintenance for Codex-only workspace organization.
- [`dev/test/check-core-evidence-harnesses.py`](dev/test/check-core-evidence-harnesses.py) — local self-test bundle for Gap BO, Gap BP, R178, and BlockFetch evidence helpers.
...
The active development arc is the remaining sister-tools port under
`crates/tools/` (13 operator binaries). The `cardano-cli` C-arc is closed from
the Rust implementation side; remaining cardano-cli work is upstream-binary
evidence and any future prioritized subcommands outside the current
40-command C-arc surface plus the post-C-arc `key verification-key` slice.
...
**Verification baseline** (2026-05-26, Rust 1.95.0): `cargo fmt --all -- --check`,
`cargo check-all`, `cargo lint`, and `cargo test-all` all pass —
**7,251 tests passing, 0 failing, 3 ignored** (7,254 listed tests total).
