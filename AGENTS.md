...
- 1102 workspace tests pass across all crates, 0 failures. Pre-existing failures in `seed_peer_registry_preserves_bootstrap_and_local_root_sources` (insert-after-sync ordering fix) and `decode_multi_era_block_byron_main` (Byron body fixture array encoding fix) are resolved.
