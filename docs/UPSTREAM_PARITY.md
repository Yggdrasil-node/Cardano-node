...
- Focused R248 TPraos overlay tests plus live preview resume to Babbage slot `412896`: passing
...
- **Preview TPraos overlay VRF parity** (R248): Shelley-family TPraos active overlay slots now classify the epoch overlay schedule and apply the upstream genesis-delegate branch: verify the selected genesis delegate cold key, the delegate VRF key, and both TPraos VRF proofs, then skip the pool stake leader-threshold check. Reserved non-active overlay slots fail closed. A live preview resume passed the former active-overlay blocker at slot `106220` and advanced to Babbage slot `412896` with no `VRF verification failed`, `MalformedReferenceScripts`, ledger decode error, or panic.
