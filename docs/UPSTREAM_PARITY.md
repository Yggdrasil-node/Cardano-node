...
Last updated: 2026-05-26; status header refreshed after R839 cardano-testnet Property/Run testnetProperty planning helpers
...
> in [`docs/COMPLETION_ROADMAP.md`](COMPLETION_ROADMAP.md) and
> [`docs/parity-matrix.json`](parity-matrix.json).
> Post-R688 operational-run evidence now reaches R839; the newer rounds
> advance sister-tool implementation surfaces and audit hygiene without
> changing the open operator/wire-evidence gates in this matrix.
>
...
> lives in [`docs/strict-mirror-audit.tsv`](strict-mirror-audit.tsv). R310
> fixed a `.gitignore` over-broad pattern; R311 added an index-vs-tree drift
> check; R313-R320 cleaned up the docstring-classification taxonomy. Later
> cleanup rounds moved stale placements and closed the `cardano-cli` C-arc
> implementation, so this block is retained as historical file-policy
> context rather than the current tool-migration status.

## Verification Baseline

### Current verification baseline

- Latest broad workspace baseline (2026-05-26, Rust 1.95.0):
...
- No additional code-level Plutus parity blocker is tracked beyond Gap BP below; Gap BP remains the active preview-tip evidence blocker for Plutus V2 budget parity.
...
Later local evidence after R249 ruled out the protocol-version cost-model
variant, per-transaction cost-model propagation, the audited per-builtin
variant-A formulas, uniform CEK step costs, slip-batch flush timing, and the
top-level V2 `ScriptContext` / `TxInfo` shape. The remaining active suspects
are deeper `ScriptContext` field encoding, redeemer/datum/input ordering, or
...
`scripts/compare-gap-bp-traces.py --require-haskell --require-equal` against
the Rust artifacts before changing CEK or context construction.
