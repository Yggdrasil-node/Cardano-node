...
The chain hashes are byte-identical between implementations, so you can hash-compare blocks at the same slot using [`dev/evidence/compare_tip_to_haskell.sh`](https://github.com/yggdrasil-node/Cardano-node/blob/main/dev/evidence/compare_tip_to_haskell.sh).
