...
- **`scripts/`** — operator scripts: `install_from_release.sh`, `healthcheck.sh`, `backup_db.sh`, `check_upstream_drift.sh`, `compare_tip_to_haskell.sh`, `parallel_blockfetch_soak.sh`, `preview_producer_harness.sh`, `restart_resilience.sh`, `run_preview_real_pool_producer.sh`, `run_mainnet_real_pool_producer.sh`, `run_preprod_real_pool_producer.sh`, `yggdrasil-node.service` (systemd unit template).
