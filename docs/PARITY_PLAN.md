...
- ✅ **Phase D.1 rollback recovery first slice** (R225 + R237 — rollback-depth histogram plus epoch-boundary-aware checkpoint replay when stake snapshots are enabled; nonce/OpCert historical sidecar rollback remains a hardening item)
...
1. **Phase D.1 rollback sidecar hardening** — preserve nonce and OpCert sidecars at the exact rollback restore point instead of relying on reset/replay behavior
2. **Phase E.1 cardano-base** — coordinated vendored fixture refresh (`git mv` of `specs/upstream-test-vectors/cardano-base/<sha>/` + corpus re-run; network-dependent)
3. **Phase E.2 24h+ mainnet rehearsal** — sustained operator wall-clock observation; sync infrastructure is end-to-end working post-R211/R213
4. **Plutus CEK drift monitoring** (ongoing — keep Conway/Plomin cost-model key mapping in sync)
...
- ✅ **Comprehensive end-to-end verification post-Phase A (Round 205)** — operational verification confirms 25/25 cardano-cli `conway query` subcommands pass on a fresh preview sync; all 3 consensus-side sidecars persist atomically; **live nonces survive node restart** (`candidateNonce`/`evolvingNonce`/`labNonce` show real Blake2b hashes loaded from `nonce_state.cbor` after recovery); sync resumes from checkpoint at slot 9960 → advances to 11940.  Phase A 6/7 complete; only A.6 (`GetGenesisConfig` ShelleyGenesis serialiser) deferred for lack of direct cardano-cli consumer.
