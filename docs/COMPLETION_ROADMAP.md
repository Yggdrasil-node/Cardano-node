...
typed `GovAction::NoConfidence`. R670 (2026-05-21) typed
...
`GovAction::NewConstitution`. R672 (2026-05-21) typed
...
(2026-05-21) typed `CollectError::NoRedeemer` (tag 0). R681
(2026-05-21) added the `ContextError` 8-variant scaffold
(`ConwayContextError` tags 8-15) and typed
`CollectError::BadTranslation` — tags 9/10/11/14/15
(CertificateNotSupported `TxCert`, PlutusPurposeNotSupported
`PlutusPurpose AsItem`, current-treasury / treasury-donation
`Coin`, ReferenceInputsNotDisjointFromInputs `NonEmpty TxIn`)
carry typed payloads; tags 8/12/13 keep raw inner CBOR.
