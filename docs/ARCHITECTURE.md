...
- Phase 2: root-set providers in `crates/network` is in progress. The crate now exposes a resolved startup snapshot, mutable root-provider state, and a refresh-oriented provider API for local, bootstrap, and public roots with disjointness and precedence handling. That refresh path can now also reconcile the peer registry directly. Next expand that layer toward real dynamic local-root, bootstrap, public-root, and later DNS- or ledger-driven providers.
- Phase 3: peer registry state in `crates/network` is started. The crate now exposes a minimal registry for peer source and status aligned with upstream `PeerSource` and `PeerStatus` concepts, including local root, public root, bootstrap, ledger, and peer-share origins plus cold, cooling, warm, and hot states. Next make provider refreshes and future peer-share or ledger inputs flow through this registry so `node` no longer carries reconnect or peer-preference state beyond reporting.
