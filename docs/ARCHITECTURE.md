...
- Phase 6: runtime multi-session orchestration. The dispatch layer + the per-peer worker primitive are now both in place; what remains is the runtime change that registers per-peer workers on connect and routes the sync-loop's BlockFetch through them. **This is no longer a consensus-correctness change** — the tentative-header timing contract is locked in `dispatch_range_with_tentative` and tested. It is a runtime architecture change that touches connection lifecycle. Stepwise progress:

  1. **Warm-peer BlockFetch handle accessor.** **Done** (commit `5d44c70`). `OutboundPeerManager::with_hot_block_fetch_clients<R>(&mut self, f: FnOnce(&mut [(SocketAddr, &mut BlockFetchClient)]) -> R) -> R` exposes hot peers' BlockFetch handles as a borrow-checked slice; `hot_peer_addrs()` is the cheap snapshot for sizing concurrency.

  2. **Sync-loop dispatch consumer.** **Pending.** Inside `sync_batch_verified_with_tentative`, after `chain_sync.request_next_typed()` returns `RollForward { header, tip }`, branch on `config.effective_block_fetch_concurrency(n_warm_hot_peers) > 1`:
     - If 1: legacy path (existing single-peer fetch via `block_fetch.request_range_collect_*`).
     - If > 1: dispatch through the registered [`crate::blockfetch_worker::FetchWorkerPool`] via `pool.dispatch_plan(plan, from_point, pool_instr)`, then thread the result through `dispatch_range_with_tentative` for the consensus-correctness contract.

  3. **Async-borrow lifetime constraint.** **Done** (commit `<this commit>`, [`node/src/blockfetch_worker.rs`](../node/src/blockfetch_worker.rs)). Resolved by per-peer worker tasks: each peer's `BlockFetchClient` is owned by its own tokio task that drains a `mpsc::Receiver<FetchRequest>` queue. The sync loop dispatches via `mpsc::Sender::send` (no `&mut BlockFetchClient` borrow crosses the await), and per-request `oneshot::Sender` returns the result. Workers run in parallel because each is its own task. Mirrors upstream `BlockFetch.ClientRegistry` per-peer `FetchClientStateVars` + STM exactly.

  4. **Connection-manager coordination.** **Pending.** Multi-peer fetch presupposes ≥2 hot peers. The governor's `evaluate_hot_promotions` already produces N promotions per tick. The `ConnectionManagerState` already tracks per-peer connection lifecycle. On peer disconnect, the runtime calls `FetchWorkerPool::unregister(&addr)` to remove the worker and `prune_closed()` to garbage-collect any worker whose channel has died. On reconnect, `FetchWorkerHandle::spawn` registers a fresh worker. The dispatcher's error-propagation path (drop pending oneshot receivers, propagate first error) is already correct for mid-fetch peer loss — workers stay alive for subsequent iterations; only the offending request's response is lost.

  5. **Operational rollout.** Default `max_concurrent_block_fetch_peers = 1` keeps the legacy path active and the change is purely additive. Operators opt in by setting the knob `> 1`. The runbook in `docs/MANUAL_TEST_RUNBOOK.md` should grow a §6.5 entry for parallel-fetch soak (2- and 4-peer) with hash-comparison vs. Haskell node before the knob default flips above 1. Upstream `bfcMaxConcurrencyBulkSync = 2` is the natural default for syncing nodes.
