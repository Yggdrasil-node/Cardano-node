...
- Phase 2: root-set providers in `crates/network` is in progress. The crate now exposes a resolved startup snapshot, mutable root-provider state, and a refresh-oriented provider API for local, bootstrap, and public roots with disjointness and precedence handling. Next expand that layer toward real dynamic local-root, bootstrap, public-root, and later DNS- or ledger-driven providers.
