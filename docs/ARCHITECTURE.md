...
- Storage crate now also exposes a minimal `ChainDb` coordination layer for best-known tip recovery, typed ledger checkpoint restore and replay, checkpoint retention/truncation, volatile-prefix promotion into immutable storage, rollback-time snapshot truncation, and opaque slot-indexed `chain_dep_state/<slot-hex>.cbor` sidecar persistence without moving sync policy into `node`.
...
- **Genesis density signal** — ChainSync `RollForward` pushes per-peer `slot` observations into `crates/consensus::DensityWindow`; the runtime `crates/node/sync::DensityRegistry` aggregates per peer; the `crates/network::governor::run_governor_loop` reads density into `PeerMetrics.density` before each tick; `combined_score` adds a `HIGH_DENSITY_BONUS` for peers above `LOW_DENSITY_THRESHOLD = 0.6`, biasing hot demotion toward laggards.
- **Hot-peer scheduling weights** — `crates/network::governor::HotPeerScheduling` carries upstream-default per-`MiniProtocolNum` weights (BlockFetch=10, ChainSync=3, TxSubmission=2, KeepAlive=1, PeerSharing=1); `crates/node/runtime::apply_hot_weights` reads from the table on every promote-to-hot call so operator overrides via `set_hot_protocol_weight` land at the next promotion; the mux writer's per-round weighted round-robin reads `WeightHandle` atomically each round so updates take effect immediately.
- **Multi-peer BlockFetch dispatch primitives** — `crates/node/sync::partition_fetch_range_across_peers` translates the `max_concurrent_block_fetch_peers` config knob into per-peer `BlockFetchAssignment`s using `crates/network::blockfetch_pool::split_range`; `crates/node/sync::execute_multi_peer_blockfetch_plan<B>` dispatches assignments concurrently via `tokio::JoinSet`, propagates errors with `abort_all`, and reassembles chunks in chain order via `ReorderBuffer<B>`; `crates/node/sync::dispatch_range_with_tentative` wraps the above in the consensus-correctness contract (announce `try_set_tentative_header` before dispatch, call `clear_tentative_trap` on any chunk failure). The dispatcher itself stays tentative-state-agnostic so async tasks cannot race on mutation — the consensus boundary lives in the single layer.

The 2026-Q2 audit ([`docs/archive/AUDIT_VERIFICATION_2026Q2.md`](archive/AUDIT_VERIFICATION_2026Q2.md)) closed every confirmed-active parity slice plus the runtime integrations originally tracked as follow-ups (`+117` cycle delta plus E-Phase6-Seam, E-Inline, E-Workers, E-Production-Spawn, E-Migration, E-Wire, E-Promote, E-Runbook, and Phase 6 observability). The 2026-Q3 operational pass on `main` then closed the [audit C-1/H-1/H-2/M-1..M-8/L-1..L-9](archive/code-audit.md) findings and surfaced + fixed the byron→shelley fee-validation parity bug at preprod slot 518 460 (see [`docs/REAL_PREPROD_POOL_VERIFICATION.md`](REAL_PREPROD_POOL_VERIFICATION.md)). R238 closes the code-level Phase D.1 rollback sidecar hardening slice: nonce/OpCert ChainDepState now restores from the newest slot-indexed sidecar at or before the rollback point and replays stored blocks to the selected target. R239 closes Phase E.1 upstream pin maintenance by refreshing the SHA-anchored `cardano-base` vector tree and aligning all 6 documentary pins with live HEAD. R246 closes the observed preview Plutus blockers through refscan slot `901725` and fixes runtime resume preservation of recovered pool block counts. R247 fixes the Origin-start verified BlockFetch prefix window so clean preview replay stores the initial Byron slots before continuing. Post-R503 operational-run evidence now reaches R839, with the broad 2026-05-26 baseline at 7,251 passing tests, 0 failing, and 3 ignored (7,254 listed tests total) across clean `cargo fmt --all -- --check`, `cargo check-all`, `cargo lint`, and `cargo test-all` gates.
...
The remaining production-readiness gates are operator-side: the mainnet sync
endurance run (`docs/MANUAL_TEST_RUNBOOK.md` §2–9), ongoing operational
hardening, and the current §6.5 parallel-fetch sign-off. Historical rounds R218
and R258 recorded multi-peer measurements and the default
`max_concurrent_block_fetch_peers = 2`, but the 2026-05-19 full-parity audit
revalidated preview and mainnet and observed healthy single-peer progress with
`yggdrasil_blockfetch_workers_registered = 0`. Treat §6.5 as open until a
fresh Haskell-compared 6h/24h run shows worker activation on the current code.
...
- Phase 6: runtime multi-session orchestration. **Status: primitives complete;
  current operational sign-off open.** End-to-end multi-peer concurrent
  BlockFetch primitives are wired, and the default knob is
  `max_concurrent_block_fetch_peers = 2` (matching upstream
  `bfcMaxConcurrencyBulkSync`). However, current live revalidation shows the
  running preset paths still syncing through one active peer with zero
  registered BlockFetch workers. Operators wanting strict single-peer behaviour
  for replay/audit set the knob to `1`. The consensus-correctness contract is
  locked in `dispatch_range_with_tentative` and tested
  (announce → dispatch → clear-trap-on-failure).
  ...
  5. **Operational rollout.** **Open on current evidence.** Default
     `max_concurrent_block_fetch_peers = 2` ships matching upstream
     `bfcMaxConcurrencyBulkSync = 2`, and R218 historically measured a 67%
     mainnet throughput delta on a multi-peer path. Current R513 revalidation
     did not reproduce worker activation from the preset runtime paths
     (`yggdrasil_blockfetch_workers_registered = 0` on bounded preview/mainnet
     diagnostics). The §6.5 parallel-fetch rehearsal (steps 6.5a–6.5f covering
     2- and 4-peer soak with hash-comparison vs. Haskell node and
     restart-resilience cycles) remains in
     [`docs/MANUAL_TEST_RUNBOOK.md`](MANUAL_TEST_RUNBOOK.md) for current
     sign-off. Phase 6 observability (`yggdrasil_blockfetch_workers_registered`
     gauge + `_migrated_total` counter) gives operator dashboards the
     instrumentation needed to alert on stuck migration.
