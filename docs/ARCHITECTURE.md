...
The 2026-Q2 audit ([`docs/AUDIT_VERIFICATION_2026Q2.md`](AUDIT_VERIFICATION_2026Q2.md)) closed every confirmed-active parity slice with **+89 tests** added in this cycle (Slice B 16 + Slice D 16 + Slice E 10 + Slice GD 15 + GD-RT 9 + GD-Governor 10 + D-Scheduler 2 + E-Dispatch 6 + E-Tentative 5).

Upstream parity testing is complete with CBOR golden round-trip tests and cross-subsystem integration tests. Wire-format field names align with official Cardano CDDL schemas. `cargo test-all -- --list` discovers 4,300+ tests across all crates.

The next architecture milestones are runtime multi-session orchestration (Phase 6 below), dedicated tracer transport/metrics export, and mainnet sync endurance testing per [`docs/MANUAL_TEST_RUNBOOK.md`](MANUAL_TEST_RUNBOOK.md).
...
- Phase 6: runtime multi-session orchestration. The single remaining post-1.0 architecture step. The dispatch layer is already in place (`node::sync::partition_fetch_range_across_peers` + `execute_multi_peer_blockfetch_plan` + `dispatch_range_with_tentative`); what remains is restructuring the runtime so it actually maintains N concurrent `PeerSession`s and feeds their `BlockFetchClient` handles into the dispatcher. **This is no longer a consensus-correctness change** — the tentative-header timing contract is locked in `dispatch_range_with_tentative` and tested. It is a runtime architecture change that touches connection lifecycle. Implementation sketch:

  1. **Warm-peer BlockFetch handle accessor.** Today `node::runtime::OutboundPeerManager` owns `warm_peers: BTreeMap<SocketAddr, ManagedWarmPeer>`, and each `ManagedWarmPeer.session` is a `PeerSession` carrying its own `BlockFetchClient`. The sync loop's outer call (`run_reconnecting_verified_sync_service_chaindb_inner`) currently consumes a single `PeerSession`. To enable multi-peer dispatch, expose a borrow-checked accessor `OutboundPeerManager::with_hot_block_fetch_clients<R>(&mut self, f: FnOnce(&mut [(SocketAddr, &mut BlockFetchClient)]) -> R) -> R` that gives the sync loop a sliced view onto every currently-hot peer's `BlockFetchClient`. The accessor is the seam — no `Arc<Mutex>` wrapper of `BlockFetchClient` is required; the closure is invoked synchronously while holding a `&mut` borrow of the manager.

  2. **Sync-loop dispatch consumer.** Inside `sync_batch_verified_with_tentative`, after `chain_sync.request_next_typed()` returns `RollForward { header, tip }`, branch on `config.effective_block_fetch_concurrency(n_warm_hot_peers) > 1`:
     - If 1: legacy path (existing single-peer fetch via `block_fetch.request_range_collect_*`).
     - If > 1: build a `fetch_one` closure that captures the sliced `BlockFetchClient` view, call `dispatch_range_with_tentative(header, tip, from_point, peer_addrs, knob, tentative_state, pool_instr, fetch_one)`, and treat the returned `Vec<(Vec<u8>, MultiEraBlock)>` exactly as the legacy path treats the result of `fetch_range_blocks_multi_era_raw_decoded`.

  3. **Async-borrow lifetime constraint.** The sync loop's `dispatch_range_with_tentative` call awaits across a `JoinSet`, which requires `'static + Send + Sync` closures. The sliced `&mut BlockFetchClient` borrow cannot escape the closure body. Resolution: factor the closure body into a synchronous "schedule" step that swaps the borrowed clients into per-task channels, then awaits over channel-completion futures. Mirrors upstream `BlockFetch.ClientRegistry` which holds long-lived `FetchClientStateVars` shared with the fetch-decision policy via `TVar`.

  4. **Connection-manager coordination.** Multi-peer fetch presupposes ≥2 hot peers. The governor's `evaluate_hot_promotions` already produces N promotions per tick. The `ConnectionManagerState` already tracks per-peer connection lifecycle. The sync loop's existing single-session reconnect logic needs to handle "one of N peers disconnected mid-fetch" — the dispatcher already aborts siblings on chunk error and propagates the failure; the runtime then either falls back to single-peer for the next tick or waits for the governor to promote a replacement.

  5. **Operational rollout.** Default `max_concurrent_block_fetch_peers = 1` keeps the legacy path active and the change is purely additive. Operators opt in by setting the knob `> 1`. The runbook in `docs/MANUAL_TEST_RUNBOOK.md` should grow a §6.5 entry for parallel-fetch soak (2- and 4-peer) with hash-comparison vs. Haskell node before the knob default flips above 1.

  Reference: upstream `Ouroboros.Network.BlockFetch.ClientRegistry` + `Ouroboros.Network.BlockFetch.Decision.fetchDecisions` + `Ouroboros.Network.BlockFetch.State.completeBlockDownload`.
