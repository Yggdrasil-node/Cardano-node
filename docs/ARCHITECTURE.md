...
Upstream parity testing is complete with CBOR golden round-trip tests and cross-subsystem integration tests. Wire-format field names align with official Cardano CDDL schemas. `cargo test --workspace` currently passes with 2862 tests across all crates.
...
- Phase 4: consensus-network bridge for ledger peers is in progress. The network crate now owns ledger-peer eligibility judgement helpers for `useLedgerAfterSlot`, latest-slot gating, ledger-state judgement, and peer-snapshot freshness. Node startup now feeds two real storage-backed inputs into that path: it loads `peerSnapshotFile` JSON, derives slot-based freshness from the recovered ChainDb tip, and also recovers the immutable-ledger view to extract directly dialable registered pool relays. Startup merges those ledger peers plus snapshot peers and appends only eligible entries to the runtime fallback list. Coordinated-storage reconnect runners now also refresh both checkpoint-derived ledger pool relays and the resolved snapshot peer set before each bootstrap attempt when `useLedgerPeers` is enabled. The remaining work is to replace the node-owned refresh orchestration with a live consensus-fed path carrying authoritative ledger-state judgements, snapshot state, and time-varying ledger peer updates into the network-owned provider layer.
