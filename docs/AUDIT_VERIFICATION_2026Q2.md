...
As of the E-Inline commit `8bd4cdf`, every `confirmed-active` row in this audit is `done`, the consensus → network → governor data flow opened by Slice GD is end-to-end live, the Slice D `HotPeerScheduling` weight surface is end-to-end live (governor → mux writer), the Slice E multi-peer dispatch layer is fully implemented (planner + parallel executor + inline executor + tentative-handling glue + Phase 6 seam in `OutboundPeerManager`), and the consensus-correctness contract for the multi-peer path is locked in `dispatch_range_with_tentative`.  The deferred-slice rationale section has been removed: there are no remaining deferred slices.  Test count delta from this audit cycle: **+98** (Slice B 16 + Slice D 16 + Slice E 10 + Slice GD 15 + GD-RT 9 + GD-Governor 10 + D-Scheduler 2 + E-Dispatch 6 + E-Tentative 5 + E-Phase6-Seam 4 + E-Inline 5) on top of the ~4,284 baseline; full workspace gates (`cargo check-all`, `cargo test-all`, `cargo lint`) green at every slice boundary.
...
- **Runtime sync-loop branching** so `dispatch_range_with_tentative` (parallel-via-spawn) or `execute_multi_peer_blockfetch_plan_inline` (sequential, FnMut) is actually invoked from `sync_batch_verified_with_tentative` when the operator opts in via `max_concurrent_block_fetch_peers > 1`. Every primitive the runtime needs is now in place and tested:
  - Plan: `partition_fetch_range_across_peers` + `effective_block_fetch_concurrency`
  - Parallel executor: `execute_multi_peer_blockfetch_plan` (`'static` closures)
  - Inline executor: `execute_multi_peer_blockfetch_plan_inline` (`FnMut` closures, borrow-checked)
  - Tentative-handling glue: `dispatch_range_with_tentative`
  - Runtime seam: `OutboundPeerManager::with_hot_block_fetch_clients` + `hot_peer_addrs`

  What remains is the runtime sync loop's branch: when `effective_block_fetch_concurrency(knob, hot_count) > 1`, call `peer_manager.with_hot_block_fetch_clients(|clients| ...)` and inside the closure call `execute_multi_peer_blockfetch_plan_inline` with a closure that maps each `&mut BlockFetchClient` to a fetch via `fetch_range_blocks_multi_era_raw_decoded`. This is plumbing — no consensus-correctness review needed (the contract is locked in `dispatch_range_with_tentative`'s tests).
