...
As of the E-Migration commit `7c06baf`, every `confirmed-active` row in this audit is `done`, the consensus → network → governor data flow opened by Slice GD is end-to-end live, the Slice D `HotPeerScheduling` weight surface is end-to-end live (governor → mux writer), and the Slice E multi-peer dispatch architecture is fully implemented in upstream-faithful shape: planner + parallel executor + inline executor + tentative-handling glue + Phase 6 seam in `OutboundPeerManager` + per-peer `FetchWorkerHandle` / `FetchWorkerPool` mirroring `Ouroboros.Network.BlockFetch.ClientRegistry` + production wire from `BlockFetchClient` to worker + runtime migration plumbing (PeerSession `Option<BlockFetchClient>` + `migrate_session_to_worker` + worker unregister-on-disconnect). The consensus-correctness contract for the multi-peer path is locked in `dispatch_range_with_tentative`. The deferred-slice rationale section has been removed: there are no remaining deferred slices.  Test count delta from this audit cycle: **+116** (Slice B 16 + Slice D 16 + Slice E 10 + Slice GD 15 + GD-RT 9 + GD-Governor 10 + D-Scheduler 2 + E-Dispatch 6 + E-Tentative 5 + E-Phase6-Seam 4 + E-Inline 5 + E-Workers 14 + E-Migration 4) on top of the ~4,284 baseline; full workspace gates (`cargo check-all`, `cargo test-all`, `cargo lint`) green at every slice boundary.
...
- **Runtime PeerSession refactor + sync-loop branching.** Every primitive the runtime needs is now in place and tested. The recommended upstream-faithful path uses the per-peer worker pattern (matches `BlockFetch.ClientRegistry`):
  ...
  What remains:
  1. ~~**PeerSession refactor.**~~ **Done** (`0f612aa`).
  2. ~~**Connection-manager coordination.**~~ **Done** for the disconnect side (`7c06baf` — `demote_to_cold` unregisters the worker). The promote-to-worker call path needs to be wired from `run_governor_loop` (or wherever the runtime decides to opt in based on the knob) — calling `peer_manager.migrate_session_to_worker(peer)` after a successful `promote_to_warm`.
  3. **Sync-loop branch.** In `sync_batch_verified_with_tentative`, when `config.effective_block_fetch_concurrency(pool.len()) > 1`, partition the range and call `peer_manager.fetch_worker_pool().dispatch_plan(...)` instead of the direct `block_fetch_mut().request_range_collect_*` path. `dispatch_range_with_tentative` already handles the tentative-state contract. **This is the last code change.** Note: requires sharing the `OutboundPeerManager` (or just its `FetchWorkerPool`) between the governor and sync runtime tasks via `Arc<RwLock<...>>` — the only architectural decision left is which sharing shape to use; the recommended path is `Arc<tokio::sync::RwLock<FetchWorkerPool<MultiEraBlock>>>` cloned into both `RuntimeGovernorConfig` and `VerifiedSyncServiceConfig` so the governor populates and the sync loop reads.
