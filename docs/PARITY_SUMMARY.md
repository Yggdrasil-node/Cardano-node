...
**Status**: 188 parity rounds completed; production-ready across all subsystems with full `cardano-cli 10.16` query parity at preprod (Shelley) + preview (Alonzo) and **every Conway-governance LSQ subcommand** (constitution, gov-state, drep-state, drep-stake-distribution, committee-state, treasury, spo-stake-distribution, proposals, ratify-state, future-pparams, stake-pool-default-vote) decoding end-to-end against `YGG_LSQ_ERA_FLOOR=6`
