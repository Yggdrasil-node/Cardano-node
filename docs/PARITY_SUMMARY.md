...
**Status**: 191 parity rounds completed; production-ready across all subsystems with full `cardano-cli 10.16` query parity at preprod (Shelley) + preview (Alonzo) and **every Conway-era LSQ subcommand** (including operational `ledger-peer-snapshot`, `protocol-state`, `ledger-state`) decoding end-to-end against `YGG_LSQ_ERA_FLOOR=6`.  R191 began the post-audit data-plumbing arc — `lastSlot` / `slotNo` now reflect live tip in `protocol-state` and `ledger-peer-snapshot`
...
| **Total (R1–R191)** | **All subsystems** | **~900** | **all production-ready; 4 744 workspace tests passing, 0 failing** |
