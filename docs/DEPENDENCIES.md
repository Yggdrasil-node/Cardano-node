...
- `hex`: pure Rust hexadecimal encode/decode for text-envelope CBOR fields, CLI byte arguments, transaction IDs, and raw protocol evidence files such as tx-generator's queried protocol-parameter CBOR. No native build requirements.
...
- `yggdrasil-crypto` (internal workspace dependency, R544 direct consumer expansion): `yggdrasil-tx-generator` depends on the existing pure-Rust Ed25519 surface to derive payment verification keys from upstream TextEnvelope signing keys while porting `Cardano.TxGenerator.UTxO.mkUTxOVariant`. This adds no new crates.io or native supply-chain surface and avoids ad-hoc key/address derivation in the tx-generator crate.
- `yggdrasil-node-genesis` (internal workspace dependency, R552 direct consumer expansion): `yggdrasil-tx-generator` depends on the existing pure-Rust genesis loader/hash verifier to load Shelley `initialFunds` for `Cardano.TxGenerator.Genesis.SecureGenesis`. This adds no new crates.io or native supply-chain surface and keeps genesis pseudo-input derivation shared with the node runtime.
- `yggdrasil-plutus` (internal workspace dependency, R556 direct consumer expansion): `yggdrasil-tx-generator` depends on the existing pure-Rust Flat decoder and CEK evaluator to port upstream `Cardano.TxGenerator.Setup.Plutus.preExecutePlutusScript` for static-budget `withCheck` runs. This adds no new crates.io or native supply-chain surface and keeps Plutus execution semantics shared with the node validator.
