...
- Verified sync service (`run_verified_sync_service`, `VerifiedSyncServiceConfig`, `VerifiedSyncServiceOutcome`) uses the multi-era verified pipeline with per-block nonce evolution tracking and optional ChainState tracking. `run_verified_sync_service_chaindb` now extends that flow across coordinated storage so stable entries can be promoted through `yggdrasil-storage::ChainDb`. Runtime also exposes `run_reconnecting_verified_sync_service_chaindb`, `run_reconnecting_verified_sync_service_chaindb_with_tracer`, and `resume_reconnecting_verified_sync_service_chaindb` so reconnecting sync can keep using peer fallback logic while coordinated storage owns stable promotion and recovery. CLI `run` now uses the coordinated resume path with file-backed storage under `storage_dir` by default.
- Coordinated-storage recovery now includes `recover_ledger_state_chaindb`, which restores the latest typed checkpoint and replays both immutable and volatile blocks into a `LedgerState`. Runtime also exposes `resume_reconnecting_verified_sync_service_chaindb` to start reconnecting verified sync from that recovered point, and that resumed path now persists refreshed typed checkpoints after successful verified batches.
