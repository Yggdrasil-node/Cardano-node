...
- Multi-era block decode for all 7 era tags (Byron through Conway). Alonzo (tag 5) uses dedicated `AlonzoBlock` (5-element format with `invalid_transactions` and TPraos header), distinct from the 4-element `ShelleyBlock` used for Shelley/Allegra/Mary (tags 2–4).
...
- ChainState integration: `multi_era_block_to_chain_entry` extracts `ChainEntry` from multi-era blocks (None for Byron), `track_chain_state` applies sync steps to `ChainState` and drains stable entries, `promote_stable_blocks` copies stable blocks from volatile to immutable store.
