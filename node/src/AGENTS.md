...
- Mempool sync eviction: `extract_tx_ids` + `evict_confirmed_from_mempool`. TxSubmission runtime wiring now includes `serve_txsubmission_request_from_mempool`, a thin helper that answers one `TxSubmissionClient` request from mempool state by advertising fee-ordered txids, relaying stored submitted-transaction bytes, or sending `MsgDone` when a blocking request arrives and the mempool is empty.
