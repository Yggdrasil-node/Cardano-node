...
use super::commands::configuration::{
    ...
    load_effective_config, preset_config_base_dir, resolve_preset_config_root,
};
...
use super::commands::validate_config::validate_config_report;
#[cfg(feature = "forge")]
...
use yggdrasil_node_config::{default_config, node_role_report};
...
fn cli_help_text_documents_0x_prefix_ergonomic() {
    ...
    fn append_help_recursive(cmd: &mut clap::Command, out: &mut String) {
        out.push_str(&cmd.render_long_help().to_string());
        for sub in cmd.get_subcommands_mut() {
            append_help_recursive(sub, out);
        }
    }
    ...
    // Flatten every subcommand help recursively, so flag descriptions
    // from deeply nested cardano-cli compatibility commands appear.
    let mut flat = String::new();
    append_help_recursive(&mut cmd, &mut flat);
...
fn decode_ntc_result_shapes_typed_json_for_new_queries() {
    ...
    // CurrentEpoch: unsigned u64, shared through the cardano-cli LSQ plan.
    let v = decode_ntc_result(&QueryCommand::CurrentEpoch, &[0x05]).expect("decode CurrentEpoch");
    assert_eq!(v["epoch"], 5);

    // EraHistory: hard-fork interpreter payload, surfaced as raw CBOR.
    let v = decode_ntc_result(&QueryCommand::EraHistory, &[0x82, 0x01, 0x02])
        .expect("decode EraHistory");
    assert_eq!(v["era_history_cbor"], "820102");
...
fn encode_ntc_query_emits_expected_tag_bytes() {
    ...
    // `[tag]` == `0x81` + one-byte-unsigned(tag); the five parametric
    ...
    let cases: &[(QueryCommand, &[u8])] = &[
        ...
        // Upstream `BlockQuery (QueryHardFork GetInterpreter)` =
        // `[0, [2, [0]]]`.
        (
...
fn write_minimal_preset_bundle(root: &Path, preset: yggdrasil_node_config::NetworkPreset) {
    let dir = root.join(preset.to_string());
    std::fs::create_dir_all(&dir).expect("preset dir");
    for filename in ["config.json", "topology.json", "shelley-genesis.json"] {
        std::fs::write(dir.join(filename), "{}\n").expect("preset marker");
    }
}

#[test]
fn preset_config_root_resolver_prefers_explicit_env_root() {
    let env_root = PathBuf::from("/operator/yggdrasil/configuration");
    let resolved = resolve_preset_config_root(
        yggdrasil_node_config::NetworkPreset::Mainnet,
        Some(env_root.clone()),
        None,
        None,
        PathBuf::from("/source/configuration"),
        &[],
    );
...
#[test]
fn preset_config_root_resolver_finds_installed_prefix_share_bundle() {
    let dir = tempfile::tempdir().expect("tempdir");
    let prefix = dir.path().join("usr-local");
    let bin_dir = prefix.join("bin");
    let share_root = prefix.join("share").join("yggdrasil").join("configuration");
    std::fs::create_dir_all(&bin_dir).expect("bin dir");
    write_minimal_preset_bundle(&share_root, yggdrasil_node_config::NetworkPreset::Preview);

    let resolved = resolve_preset_config_root(
        yggdrasil_node_config::NetworkPreset::Preview,
        None,
        Some(bin_dir.join("yggdrasil-node")),
        None,
        dir.path().join("source").join("configuration"),
        &[],
    );

    assert_eq!(resolved, share_root);
}
