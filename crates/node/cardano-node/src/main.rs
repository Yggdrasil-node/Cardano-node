...
fn main() -> Result<()> {
    #[cfg(windows)]
    {
        let handle = std::thread::Builder::new()
            .name("yggdrasil-node-main".to_owned())
            .stack_size(8 * 1024 * 1024)
            .spawn(run_main)
            .expect("failed to spawn yggdrasil-node main thread");
        match handle.join() {
            Ok(result) => result,
...
fn run_main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::SubmitTx {
            ...
            run_submit_tx(socket_path, network_magic, tx_bytes)
        }
    }
}

// Shutdown signal handler moved to `crates/node/cardano-node/src/handlers/shutdown.rs` in R257
