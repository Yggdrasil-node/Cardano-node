//! Golden test: yggdrasil-cardano-submit-api `--help` / `--version`
//! outputs are byte-equivalent to the upstream `cardano-submit-api`
//! binary.
...
fn cargo_bin() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_cardano-submit-api"))
}
...
#[test]
fn help_short_flag_matches_upstream() {
    let output = Command::new(cargo_bin()).arg("-h").output().expect("spawn");
    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).expect("utf-8");
    assert_eq!(stdout, UPSTREAM_HELP);
}
