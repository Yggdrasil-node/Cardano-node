...
- Peer governor module (`governor.rs`) implements a pure decision engine with upstream-style independent targets: `GovernorTargets` now carries both regular targets (`target_known` / `target_established` / `target_active`) and big-ledger targets (`target_known_big_ledger` / `target_established_big_ledger` / `target_active_big_ledger`). `GovernorAction` remains PromoteToWarm/PromoteToHot/DemoteToWarm/DemoteToCold. Evaluation functions cover both regular and big-ledger promotion/demotion paths, plus local-root valency enforcement and combined `governor_tick`. `GovernorState` carries mutable failure-tracking (record_success/record_failure, is_backing_off, filter_backed_off) and churn timing (ChurnConfig, evaluate_churn, tick).
...
- Node-to-client protocol state machines and typed clients are now present: `LocalStateQuery`, `LocalTxMonitor`, and `LocalTxSubmission` mini-protocol modules plus `LocalStateQueryClient`, `LocalTxMonitorClient`, and `LocalTxSubmissionClient`. `LocalTxMonitor::MsgAcquired` follows upstream and carries only the snapshot slot; mempool size/capacity remains a separate `MsgReplyGetSizes` query.
- `ntc_peer.rs` currently provides NtC version-data types plus explicit `ntc_connect()` / `ntc_accept()` unsupported errors. Full Unix-socket NtC connection lifecycle is deferred until the mux grows non-TCP bearer support; do not treat NtC transport as production-ready yet.
...
- Next implementation order:
	...
	3. Add Unix-socket bearer support to the mux so NtC transport can move beyond the current explicit unsupported stub.
	4. Add consensus-network bridge inputs for ledger peers, latest-slot gating, and peer snapshot freshness.
	5. Expand typed protocol payload decoding (replace remaining opaque `Vec<u8>` payloads where practical).
