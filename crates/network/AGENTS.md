...
- Wire tags match upstream CDDL. ChainSync and BlockFetch now expose typed point/range helpers (`request_next_typed`, `find_intersect_points`, `request_range_points`) backed by ledger `Point` CBOR, while headers, blocks, and TxSubmission payloads remain opaque `Vec<u8>`.
- Next: continue replacing remaining opaque protocol payloads (headers, block ranges beyond points, TxSubmission payloads) with ledger/domain types where practical, then peer lifecycle management (reconnection, peer registry) and higher-level sync orchestration across peers.
