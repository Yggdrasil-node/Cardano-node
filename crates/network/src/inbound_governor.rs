...
//! ## Naming parity
//!
//! **Strict mirror:** Ouroboros/Network/InboundGovernor.hs.
//! Filename flattens the upstream directory; the file carries the
//! runtime decision engine (the `step` function + event handlers
//! + `verify_remote_transition` predicate). The state-record
//! definitions (`InboundConnectionEntry`, `InboundGovernorState`)
//! live in the sibling `inbound_governor/state.rs` mirroring
//! upstream's `InboundGovernor/State.hs`.

pub mod state;

pub use state::{InboundConnectionEntry, InboundGovernorState};
