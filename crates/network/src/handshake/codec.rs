...
//! ## Naming parity
//!
//! **Strict mirror:** Ouroboros/Network/Protocol/Handshake/Codec.hs.
//! Filename flattens the upstream directory; the file carries the
//! `to_cbor` / `from_cbor` round-trip for `HandshakeMessage` (the
//! protocol envelope), the `RefuseReason` inner-tag codec, and the
//! v7-v15 backward-compatible decoder for `NodeToNodeVersionData`
//! that accepts 2/3/4-element arrays for older peers.
...
pub(super) fn encode_version_data(enc: &mut Encoder, vd: &NodeToNodeVersionData) {
    enc.array(4)
...
pub(super) fn decode_version_data(
    dec: &mut Decoder<'_>,
) -> Result<NodeToNodeVersionData, LedgerError> {
    let len = dec.array()?;
    if !(2..=4).contains(&len) {
