...
- Root-set provider work has started here as well: topology parsing now feeds a resolved provider snapshot, mutable `RootPeerProviderState`, and a refresh-oriented provider API that enforce local-root versus bootstrap/public-root precedence and disjointness. Provider refreshes can also reconcile the `PeerRegistry` on the same crate-owned path, and a DNS-backed root-peer provider now re-resolves configured local-root, bootstrap, and public-root access points without involving `node`.
- A minimal peer registry now lives here too: `PeerRegistry` tracks `PeerSource` and `PeerStatus` per peer and can reconcile root-provider snapshots into root-peer entries while preserving non-root sources.
- Next: typed protocol payload decoding (replace remaining opaque `Vec<u8>` payloads where practical), then add richer DNS refresh policy (TTL, backoff) and extend toward ledger-driven providers, and teach the peer registry about ledger/peer-share updates before implementing any governor-style promotion or churn behavior.
