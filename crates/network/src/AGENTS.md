...
- Next: typed protocol payload decoding (replace remaining opaque `Vec<u8>` payloads where practical), then extend toward ledger-driven providers that feed those registry sync paths before implementing any governor-style promotion or churn behavior.
