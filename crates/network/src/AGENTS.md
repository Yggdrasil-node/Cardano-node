...
- Root-set provider work has started here as well: topology parsing now feeds a resolved provider snapshot and mutable `RootPeerProviderState` that enforce local-root versus bootstrap/public-root precedence and disjointness.
- Next: typed protocol payload decoding (replace remaining opaque `Vec<u8>` payloads where practical), then extend the root-set layer toward real refresh-driven providers and add peer registry state before implementing any governor-style promotion or churn behavior.
