...
- Next: typed protocol payload decoding (replace remaining opaque `Vec<u8>` payloads where practical), then extend toward ledger-driven providers and teach the peer registry about ledger/peer-share updates before implementing any governor-style promotion or churn behavior.
