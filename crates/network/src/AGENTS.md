...
- Peer governor module (`governor.rs`) now provides a pure decision engine with upstream-style independent target sets: `GovernorTargets` includes regular targets (`target_known`/`target_established`/`target_active`) and big-ledger targets (`target_known_big_ledger`/`target_established_big_ledger`/`target_active_big_ledger`). `LocalRootTargets` stays per-group warm/hot valency from `LocalRootConfig`, and `GovernorAction` remains PromoteToWarm/PromoteToHot/DemoteToWarm/DemoteToCold. Evaluation functions now cover both regular and big-ledger promotion/demotion paths, plus `enforce_local_root_valency` and `governor_tick`. `GovernorState` carries mutable failure-tracking (`record_success`/`record_failure`, `is_backing_off`, `filter_backed_off`) and churn timing (`ChurnConfig`, `evaluate_churn`, `tick`).
...
- Node-to-client protocol implementation work is now in-tree too: `protocols/local_state_query.rs`, `protocols/local_tx_monitor.rs`, `protocols/local_tx_submission.rs`, plus their typed clients. `LocalTxMonitor::MsgAcquired` intentionally models the upstream slot-only wire payload; callers must use `MsgGetSizes` for capacity/usage data.
- `ntc_peer.rs` is currently limited to version-data helpers and explicit unsupported transport stubs because `mux::start()` only accepts `TcpStream`. Full Unix-socket NtC connection support should land only once there is a non-TCP bearer path in the mux.
- Next: integrate governor `tick()` into the node runtime loop to drive peer promotions/demotions, wire PeerSharing client into governor for peer discovery, then add Unix-socket bearer support for NtC transport.
