...
- Runtime integration status: the node runtime now opens mini-protocol 10 on standard node-to-node sessions, advertises `peer_sharing = 1` during bootstrap proposals, and can reconcile peer-sharing discoveries into `PeerRegistry` as `PeerSourcePeerShare`. `PeerRegistry` now also tracks per-peer `hot_tip_slot` metadata and exposes both `preferred_hot_peer()` and `hot_peers_by_reconnect_priority()` so reconnect paths can prefer and front-load the most advanced known hot peers. `set_hot_tip_slot()` only applies to peers currently marked `PeerHot`, preventing stale tip metadata on warm/cold entries. Remaining work is full hot-protocol scheduling beyond reconnect bootstrap preference and any additional responder-side runtime usage.
