...
- [Storage modules (ChainDB, ImmutableDB, VolatileDB, LedgerDB)](../../.reference-haskell-cardano-node/deps/ouroboros-consensus/ouroboros-consensus/src/ouroboros-consensus/Ouroboros/Consensus/Storage/)
- [ChainDB coordination](../../.reference-haskell-cardano-node/deps/ouroboros-consensus/ouroboros-consensus/src/ouroboros-consensus/Ouroboros/Consensus/Storage/ChainDB/)
- [ImmutableDB](../../.reference-haskell-cardano-node/deps/ouroboros-consensus/ouroboros-consensus/src/ouroboros-consensus/Ouroboros/Consensus/Storage/ImmutableDB/)
- [VolatileDB](../../.reference-haskell-cardano-node/deps/ouroboros-consensus/ouroboros-consensus/src/ouroboros-consensus/Ouroboros/Consensus/Storage/VolatileDB/)
- [LedgerDB](../../.reference-haskell-cardano-node/deps/ouroboros-consensus/ouroboros-consensus/src/ouroboros-consensus/Ouroboros/Consensus/Storage/LedgerDB/)
- [Consensus tech report (storage design rationale)](https://ouroboros-consensus.cardano.intersectmbo.org/pdfs/report.pdf)
- [Consensus documentation and architecture notes](../../.reference-haskell-cardano-node/deps/ouroboros-consensus/docs/)
- [Node integration reference for operational storage concerns](../../.reference-haskell-cardano-node/cardano-node/)
...
- `ocert_sidecar` module exposes opaque atomic sidecar helpers for consensus-adjacent state. Slot-indexed `chain_dep_state/<slot-hex>.cbor` snapshots are the canonical nonce/OpCert ChainDepState history via `save_chain_dep_state_snapshot`, `load_latest_chain_dep_state_snapshot_before_or_at`, `truncate_chain_dep_state_snapshots_after`, and `retain_latest_chain_dep_state_snapshots`; `stake_snapshots.cbor` remains the separate stake-snapshot mirror. Storage never decodes these bytes; `node` owns the typed ChainDepState bundle. The write discipline remains write-temp-then-rename plus parent-dir-fsync on Unix, mirroring upstream `Ouroboros.Consensus.Storage.LedgerDB` snapshot durability and rollback restore/replay behavior where the host platform exposes portable directory fsync.
...
- `ImmutableStore::iter_after(&Point) -> Result<Box<dyn Iterator<Item = Block>>, _>` (R482) is the streaming counterpart to `suffix_after`. Default trait impl delegates to `suffix_after().into_iter()`; `InMemoryImmutable` and `FileImmutable` override with lazy-yield via `chain[start..].iter().cloned()` (no intermediate `Vec`). The R482 refactor also extracts a private `resolve_suffix_start(&self, point) -> Result<usize, StorageError>` helper on both implementations, so `suffix_after` and `iter_after` now share the same 3-case point-resolution logic (Origin → 0; outside range → 0 or chain.len(); inside range → pos + 1 or `PointNotFound`) and are guaranteed to yield byte-identical sequences (verified by per-backend `*_iter_and_suffix_yield_same_sequence` tests). `db-analyser`'s `lib.rs::run` consumes `iter_after` for multi-terabyte forensic chain walks. Reference: upstream `Ouroboros.Consensus.Storage.ImmutableDB.API.Iterator`. Carve-out surviving R482: the file-backed `FileImmutable` still loads every block into `self.index: HashMap<HeaderHash, Block>` at open time; a future arc may redesign the on-disk layout for true lazy block loading.
...
- `recover_ledger_state()` uses a fallback strategy: when the latest checkpoint is corrupt (CBOR decode failure), it iterates backward through older snapshots via `try_restore_checkpoint()` until a valid one is found, falling through to the base ledger state when all checkpoints are unreadable. This makes recovery resilient to partial or interrupted checkpoint writes. After any valid typed checkpoint restore, rehydrate genesis-derived runtime fields from the caller-provided base ledger state before replay; checkpoint CBOR omits those fields and reward calculation depends on `maxLovelaceSupply` for upstream circulation parity.
- File-backed stores (`FileImmutable`, `FileVolatile`, `FileLedgerStore`) use atomic file writes (write-to-temp + `fs::rename`) for crash safety, preventing corrupt on-disk state from incomplete writes. Parent-directory fsync is performed on Unix; Windows builds skip that fsync because `std::fs::File::open` cannot portably open directories there.
...
- Active crash recovery on stale `dirty.flag`: all three file-backed stores (`FileImmutable`, `FileVolatile`, `FileLedgerStore`) actively recover when a stale sentinel is found on write-owning `open()` — incomplete `.tmp` files from interrupted atomic writes are deleted, volatile delete WALs are replayed, and the dirty flag itself is cleared after the recovery scan completes so subsequent opens do not emit spurious warnings. Read-only operator inspection must use `open_read_only()` so `status`/`validate-config` can scan a possibly live store without deleting temp files, applying WALs, or clearing `dirty.flag`.
