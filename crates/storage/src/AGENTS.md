...
- [ChainDB orchestration](../../../.reference-haskell-cardano-node/deps/ouroboros-consensus/ouroboros-consensus/src/ouroboros-consensus/Ouroboros/Consensus/Storage/ChainDB)
...
- [Consensus storage documentation](../../../.reference-haskell-cardano-node/deps/ouroboros-consensus/docs)
...
- `recover_ledger_state()` uses `try_restore_checkpoint()` to iterate backward through available snapshots when the latest checkpoint is corrupt, providing resilience against partial or interrupted writes. Decoded ledger checkpoints intentionally omit genesis/runtime fields; every successful restore must call `LedgerState::rehydrate_runtime_genesis_from(&base_state)` before replay so reward circulation, epoch sizing, PPUP stability-window logic, and pre-Shelley genesis bootstrap remain configured.
