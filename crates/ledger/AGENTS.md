...
- `Block` and `BlockHeader` use typed identifiers; `LedgerState` tracks tip via `Point`, owns dual UTxO sets (`ShelleyUtxo` legacy + `MultiEraUtxo` generalized), and now carries explicit stake-pool plus reward-account state containers for upcoming certificate and withdrawal work.
