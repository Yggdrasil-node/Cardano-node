...
pub fn extract_block_tx_byte_spans(raw_block: &[u8]) -> Result<BlockTxRawSpans, LedgerError> {
    ...
    // The era-tagged outer wrapper `[era_tag, block]` may be either a
    // definite-length 2-array or, in rare cases, indefinite-length. Accept
    // both shapes here for the same reason we accept indefinite-length
    // arrays everywhere else in this function — preview Babbage blocks
    // observed on the IOG bootstrap peer (e.g. block at slot ~1,525,259 in
    // chunk 353) carry indefinite-length encodings (CBOR additional-info
    // 31, byte `0x9f`), and any strict `array()` call here would force the
    // apply path to fall back to `tx_body.to_cbor_bytes()` re-serialization
    // — defeating R85's byte-canonical preservation and producing a
    // wrong `tx_body_hash` against which any vkey witness signature
    // verifies invalid (R251 Gap BQ).
    let outer_envelope = envelope.array_begin()?;
    let envelope_finite = outer_envelope == Some(2);
    if envelope_finite && envelope.unsigned().is_ok() {
        let inner = envelope.raw_value()?;
        if envelope.is_empty() || envelope.is_break() {
    ...
    // Accept both definite-length and indefinite-length encodings.
    let outer_len = dec.array_begin()?;
    ...
    dec.skip()?;
    // Element 1: array of transaction bodies — definite or indefinite.
    let bodies = collect_indefinite_or_definite_spans(&mut dec)?;
    // Element 2: array of witness sets — definite or indefinite.
    let witness_sets = collect_indefinite_or_definite_spans(&mut dec)?;
    Ok(BlockTxRawSpans {
        bodies,
        witness_sets,
    })
...
/// Walk a CBOR array (definite-length OR indefinite-length per RFC 8949
/// §3.2.1) and capture the on-wire byte span of each element. Returns the
/// per-element raw bytes preserved exactly as they appear in the input.
///
/// Indefinite-length arrays are signalled by initial byte `0x9f` (major
/// type 4 with additional info 31) and terminated by the break stop-code
/// `0xff`. Both encodings are valid CBOR and both appear on Cardano blocks
/// in the wild — preview Babbage blocks observed during R251 use the
/// indefinite form on body and witness-set arrays.
fn collect_indefinite_or_definite_spans(
    dec: &mut Decoder<'_>,
) -> Result<Vec<Vec<u8>>, LedgerError> {
    match dec.array_begin()? {
        Some(count) => {
            let mut out: Vec<Vec<u8>> = vec_with_safe_capacity(count, BLOCK_BODY_ELEMENTS_MAX);
            for _ in 0..count {
                let start = dec.position();
                dec.skip()?;
                let end = dec.position();
                out.push(dec.slice(start, end)?.to_vec());
            }
            Ok(out)
        }
        None => {
            ...
            while !dec.is_break() {
                if out.len() >= BLOCK_BODY_ELEMENTS_MAX {
                    return Err(LedgerError::CborInvalidLength {
                        expected: BLOCK_BODY_ELEMENTS_MAX,
                        actual: out.len() + 1,
                    });
                }
                let start = dec.position();
