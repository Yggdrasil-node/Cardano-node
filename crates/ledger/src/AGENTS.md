...
- Per-era `apply_block()` native script validation: Allegra, Mary, Alonzo, Babbage, and Conway inner loops call `validate_native_scripts_if_present()` to evaluate required native scripts (from script-hash payment credentials in spending inputs, certificates, and withdrawals). Missing native scripts are silently skipped (they may be Plutus scripts validated in a separate phase).
