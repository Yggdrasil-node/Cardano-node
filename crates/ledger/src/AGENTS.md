...
- `LedgerState` owns dual UTxO sets: `ShelleyUtxo` (legacy, for backward compat) and `MultiEraUtxo` (generalized). It also carries explicit `PoolState` and `RewardAccounts` containers plus `LedgerStateSnapshot` read-only views. `apply_block()` dispatches per era with atomic block application and full CBOR decode + UTxO validation.
