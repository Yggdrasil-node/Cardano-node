...
impl ConwayTxBody {
    /// Decode just enough of a Conway tx body to return the output count.
    ///
    ...
    ///
    /// Mirror of upstream `Cardano.Tools.DBAnalyser.HasAnalysis.Conway`
    ...
    /// Decode the input list from a Conway tx body.
    ///
...
mod tests {
    ...
    fn decode_output_count_single_output() {
        ...
        assert_eq!(ConwayTxBody::decode_output_count(&bytes).unwrap(), 1);
    }

    #[test]
    fn decode_output_count_multiple_outputs() {
        let mut body = mk_conway_body();
        body.outputs.push(mk_babbage_txout());
        body.outputs.push(mk_babbage_txout());
        let bytes = body.to_cbor_bytes();
        assert_eq!(ConwayTxBody::decode_output_count(&bytes).unwrap(), 3);
    }

    #[test]
    fn decode_output_count_rejects_malformed_body() {
        let garbage = [0xFF, 0x00, 0x01];
        assert!(ConwayTxBody::decode_output_count(&garbage).is_err());
    }
}
