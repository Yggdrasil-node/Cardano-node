# Guidance for per-era ledger modules and era transition boundaries
...
- [Byron CDDL spec](../../../../.reference-haskell-cardano-node/deps/cardano-ledger/eras/byron/ledger/impl/cddl-spec)
...
- Mary: `Value` (coin/multi-asset), `MultiAsset`, `MintAsset`, `MaryTxOut`, `MaryTxBody` (keys 0–9 including certificates/withdrawals/update using `ShelleyUpdate`) with CBOR codecs. Unsigned `MultiAsset` decode follows upstream pre-Conway `decodeWithPrunning`: zero quantities are pruned and policies made empty by pruning are omitted before UTxO validation.
