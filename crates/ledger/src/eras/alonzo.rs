...
impl AlonzoTxBody {
    /// Decode just enough of an Alonzo tx body to return the output count.
    ///
    ...
    ///
    /// Mirror of upstream `Cardano.Tools.DBAnalyser.HasAnalysis.Alonzo`
    ...
    /// Decode the input list from an Alonzo tx body.
    ///
...
mod tests {
    ...
    fn decode_output_count_single_output() {
        ...
        assert_eq!(AlonzoTxBody::decode_output_count(&bytes).unwrap(), 1);
    }

    #[test]
    fn decode_output_count_multiple_outputs() {
        let mut body = mk_alonzo_body();
        body.outputs.push(mk_alonzo_txout());
        body.outputs.push(mk_alonzo_txout());
        body.outputs.push(mk_alonzo_txout());
        let bytes = body.to_cbor_bytes();
        assert_eq!(AlonzoTxBody::decode_output_count(&bytes).unwrap(), 4);
    }

    #[test]
    fn decode_output_count_rejects_malformed_body() {
        let garbage = [0xFF, 0x00, 0x01];
        assert!(AlonzoTxBody::decode_output_count(&garbage).is_err());
    }
}
