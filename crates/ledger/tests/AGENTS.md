...
- Shelley ledger tests: <../../../.reference-haskell-cardano-node/deps/cardano-ledger/eras/shelley/test-suite/test>
- Alonzo ledger tests: <../../../.reference-haskell-cardano-node/deps/cardano-ledger/eras/alonzo/test-suite/test>
- Conway ledger tests: <../../../.reference-haskell-cardano-node/deps/cardano-ledger/eras/conway/impl/test>
- Per-era CDDL conformance data: <../../../.reference-haskell-cardano-node/deps/cardano-ledger/eras/> (each era has `impl/cddl/data/`)
...
- Deposit preservation tests (`deposit_preservation.rs`): 10 tests covering deposit/refund inclusion in the upstream value preservation equation (`consumed + refunds + withdrawals = produced + fee + deposits [+ donation]`). Coverage: Shelley key registration deposit, registration-without-deposit failure, key deregistration refund, Conway explicit-deposit registration/deregistration, pool registration deposit, combined deposit+withdrawal, Conway deposit+donation, DRep registration deposit, DRep deregistration refund.
- Conway certificate deposit validation tests (`conway_cert_deposit_validation.rs`): 15 tests covering upstream DELEG/GOVCERT deposit rules — key deposit mismatch phase split (`IncorrectDepositDELEG` in bootstrap PV 9, `DepositIncorrectDELEG` in post-bootstrap PV 10+), key refund mismatch phase split (`IncorrectKeyDepositRefund` bootstrap, `RefundIncorrectDELEG` post-bootstrap), `DrepIncorrectDeposit` (DRep deposit mismatch), `DrepIncorrectRefund` (DRep refund mismatch), `WithdrawalNotFullDrain` (exact-drain withdrawal semantics), combined registration+delegation deposit checks, Shelley-era backward-compatibility (legacy certs accepted without deposit validation).
- Dormant epoch tracking tests (`dormant_epoch.rs`): 6 tests covering upstream `numDormantEpochs` — CBOR round-trip (22-element array), epoch boundary increment when no proposals remain, epoch boundary reset when proposals exist (with DRep-stake-seeded ratification that blocks premature ratification), per-tx DRep expiry bump when proposals appear (`updateDormantDRepExpiries`), dormant counter preservation for proposal-less transactions, skip for fully expired DReps. Conway proposal deposits in value preservation equation (`totalTxDeposits = certDeposits + proposalDeposits`).
