...
- Conway certificate deposit validation tests (`conway_cert_deposit_validation.rs`): 14 tests covering upstream DELEG/GOVCERT deposit rules — `IncorrectDepositDELEG` (key deposit mismatch), `IncorrectKeyDepositRefund` (key refund mismatch), `DrepIncorrectDeposit` (DRep deposit mismatch), `DrepIncorrectRefund` (DRep refund mismatch), `WithdrawalNotFullDrain` (exact-drain withdrawal semantics), combined registration+delegation deposit checks, Shelley-era backward-compatibility (legacy certs accepted without deposit validation).
