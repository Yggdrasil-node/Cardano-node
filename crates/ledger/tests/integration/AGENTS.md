...
- `witness_validation.rs`: end-to-end `apply_block()` validation coverage for VKey witness sufficiency, Ed25519 signature verification, native script evaluation, and Conway governance guardrails including bootstrap proposal allow-reject paths (`HardForkInitiation` and `ParameterChange` acceptance plus non-bootstrap rejection), bootstrap vote allow-reject paths (DRep `InfoAction`, committee/SPO bootstrap-action acceptance, and non-bootstrap rejection), voter-existence checks, and governance-action validation. All signing-based tests use real Ed25519 signing via `yggdrasil_crypto::ed25519::SigningKey`.
