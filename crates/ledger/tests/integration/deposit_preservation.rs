...
fn shelley_key_registration_deposit_balances_value_preservation() {
    ...
    let block = make_shelley_block_with_txs(5, 1, 0x02, vec![ShelleyTxBody {
        inputs: vec![ShelleyTxIn { transaction_id: [0x01; 32], index: 0 }],
        outputs: vec![ShelleyTxOut { address: vec![0x02], amount: 1_000_000 }],
        fee,
        ttl: 100,
    ...
    state.apply_block(&block).expect("block with key registration should balance");
...
fn shelley_key_registration_without_deposit_fails() {
    ...
    state.utxo_mut().insert(
        ...
        ShelleyTxOut { address: vec![0x01], amount: 1_200_000 },
    );

    let cred = StakeCredential::AddrKeyHash([0x11; 28]);
    let block = make_shelley_block_with_txs(5, 1, 0x04, vec![ShelleyTxBody {
...
fn shelley_key_deregistration_refund_balances_value_preservation() {
    ...
    state.utxo_mut().insert(
        ...
        ShelleyTxOut { address: vec![0x01], amount: fee },
    );

    let block = make_shelley_block_with_txs(5, 1, 0x06, vec![ShelleyTxBody {
        inputs: vec![ShelleyTxIn { transaction_id: [0x05; 32], index: 0 }],
        outputs: vec![ShelleyTxOut { address: vec![0x02], amount: key_deposit }],
        fee,
        ttl: 100,
    ...
    state.apply_block(&block).expect("block with key deregistration refund should balance");
    assert!(!state.stake_credentials().is_registered(&cred));
}
...
fn conway_explicit_deposit_registration_balances() {
    ...
    let block = make_conway_block(10, 1, 0x08, vec![ConwayTxBody {
    ...
    state.apply_block(&block).expect("Conway explicit deposit registration should balance");
    assert!(state.stake_credentials().is_registered(&cred));
}
...
fn conway_explicit_deposit_deregistration_refund_balances() {
    ...
    state.multi_era_utxo_mut().insert_shelley(
        ShelleyTxIn { transaction_id: [0x09; 32], index: 0 },
        ShelleyTxOut { address: vec![0x01], amount: 200_000 },
    );

    let block = make_conway_block(10, 1, 0x0A, vec![ConwayTxBody {
    ...
    state.apply_block(&block).expect("Conway explicit deposit deregistration refund should balance");
    assert!(!state.stake_credentials().is_registered(&cred));
}
...
fn shelley_pool_registration_deposit_balances() {
    ...
    state.utxo_mut().insert(
        ShelleyTxIn { transaction_id: [0x0B; 32], index: 0 },
        ShelleyTxOut { address: vec![0x01], amount: consumed },
    );
    ...
    let block = make_shelley_block_with_txs(5, 1, 0x0C, vec![ShelleyTxBody {
    ...
    state.apply_block(&block).expect("pool registration deposit should balance");
...
fn shelley_registration_deposit_plus_withdrawal_balances() {
    ...
    let ra = RewardAccount { network: 1, credential: cred_existing };
    state.reward_accounts_mut().insert(ra, RewardAccountState::new(500_000, None));
    ...
    state.utxo_mut().insert(
        ShelleyTxIn { transaction_id: [0x0D; 32], index: 0 },
        ShelleyTxOut { address: vec![0x01], amount: consumed },
    );
    ...
    let block = make_shelley_block_with_txs(5, 1, 0x0E, vec![ShelleyTxBody {
    ...
    state.apply_block(&block).expect("deposit + withdrawal should balance");
...
fn conway_deposit_plus_donation_balances() {
    ...
    state.multi_era_utxo_mut().insert_shelley(
        ShelleyTxIn { transaction_id: [0x0F; 32], index: 0 },
        ShelleyTxOut { address: vec![0x01], amount: consumed },
    );

    let cred = StakeCredential::AddrKeyHash([0x60; 28]);
    let block = make_conway_block(10, 1, 0x10, vec![ConwayTxBody {
        inputs: vec![ShelleyTxIn { transaction_id: [0x0F; 32], index: 0 }],
        outputs: vec![BabbageTxOut {
            address: vec![0x02],
            amount: Value::Coin(1_000_000),
            datum_option: None,
    ...
    state.apply_block(&block).expect("Conway deposit + donation should balance");
    assert!(state.stake_credentials().is_registered(&cred));
}
...
fn conway_drep_registration_deposit_balances() {
    ...
    let block = make_conway_block(10, 1, 0x12, vec![ConwayTxBody {
    ...
    state.apply_block(&block).expect("DRep registration deposit should balance");
...
fn conway_drep_deregistration_refund_balances() {
    ...
    state.multi_era_utxo_mut().insert_shelley(
        ShelleyTxIn { transaction_id: [0x13; 32], index: 0 },
        ShelleyTxOut { address: vec![0x01], amount: 200_000 },
    );

    let block = make_conway_block(10, 1, 0x14, vec![ConwayTxBody {
        inputs: vec![ShelleyTxIn { transaction_id: [0x13; 32], index: 0 }],
        outputs: vec![BabbageTxOut {
            address: vec![0x02],
