...
- `OcertCounters` (upstream `PraosState.csCounters` / `currentIssueNo` from `Ouroboros.Consensus.Protocol.Praos`) now implements `CborEncode`/`CborDecode` against the ledger crate's deterministic `Encoder`/`Decoder` so the per-pool monotonicity high-water marks can be persisted as a sidecar between runs. Encoding is a single CBOR map keyed by the 28-byte pool key hash with `u64` sequence-number values, emitted in canonical `BTreeMap` (lexicographic) key order so two counter maps built via different insertion orders produce identical bytes. The sidecar is written and restored by `yggdrasil_storage::{save_ocert_counters, load_ocert_counters}`; node startup loads it via `ResumeReconnectingVerifiedSyncRequest::with_ocert_persist_dir(...)` so a restart preserves the security property `stored ≤ new_seq ≤ stored + 1` rather than resetting to zero. End-to-end coverage in `tests/integration.rs::ocert_counters_persist_across_simulated_restart` exercises encode → save → load → decode → continued-validation.
