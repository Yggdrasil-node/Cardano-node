//! Operational certificate (OpCert) struct and KES period helpers.
//!
//! Mirrors upstream `Cardano.Protocol.TPraos.OCert::OCert` and the
//! `kesPeriod` / `checkKESPeriod` helpers in the same module.
//!
//! Three public items:
//!
//! - `OpCert` — operational certificate struct binding cold key → hot KES key.
//! - `kes_period_of_slot` — `slot / slots_per_kes_period` with safe div-by-zero.
//! - `check_kes_period` — verify current KES period falls within the
