...
// ---------------------------------------------------------------------------
// OcertCounters cross-restart sidecar round-trip
// ---------------------------------------------------------------------------

/// End-to-end test pinning the cross-restart persistence contract for the
/// per-pool OpCert counter map. Without this, the storage-side sidecar I/O
/// helpers and the consensus-side CBOR codec could each pass their own
/// unit tests while the composition silently drifts (e.g. the encoder
/// emitting bytes the decoder can't read, or path semantics changing).
///
/// Reference: `PraosState.csCounters` in
/// `Ouroboros.Consensus.Protocol.Praos`, restored from the `ChainDepState`
/// snapshot at node startup so monotonicity holds across restarts.
#[test]
fn ocert_counters_persist_across_simulated_restart() {
    use yggdrasil_consensus::OcertCounters;
    use yggdrasil_ledger::cbor::{CborDecode, CborEncode};
    use yggdrasil_storage::{load_ocert_counters, save_ocert_counters};

    let dir = tempfile::TempDir::new().unwrap();

    // Run 1: build counters from a sequence of accepted blocks, persist
    // sidecar atomically.
    let mut session_one = OcertCounters::new();
    session_one
        .validate_and_update([0xa1; 28], 0, true)
        .unwrap();
    session_one
        .validate_and_update([0xa1; 28], 1, true)
        .unwrap();
    session_one
        .validate_and_update([0xb2; 28], 0, true)
        .unwrap();
    session_one
        .validate_and_update([0xc3; 28], 7, true)
        .unwrap();
    save_ocert_counters(dir.path(), &session_one.to_cbor_bytes()).unwrap();

    // Run 2: simulate node restart — load sidecar bytes, decode, and
    // continue the monotonicity check from where the previous session
    // left off.
    let bytes = load_ocert_counters(dir.path())
        .unwrap()
        .expect("sidecar must be present after save");
    let mut session_two = OcertCounters::from_cbor_bytes(&bytes).unwrap();
    assert_eq!(session_two.get(&[0xa1; 28]), Some(1));
    assert_eq!(session_two.get(&[0xb2; 28]), Some(0));
    assert_eq!(session_two.get(&[0xc3; 28]), Some(7));

    // The replayed counter rejects an OpCert sequence number lower than
    ...
    // Forward progress (stored + 1) is still accepted.
    assert!(session_two.validate_and_update([0xa1; 28], 2, true).is_ok());
}

#[test]
fn ocert_counters_load_returns_none_when_no_prior_run() {
    use yggdrasil_storage::load_ocert_counters;

    let dir = tempfile::TempDir::new().unwrap();
    // Fresh storage dir — no prior `ocert_counters.cbor` exists, so the
    // restart-time loader returns `None` and startup falls back to an
    // empty counter map. This is the documented contract for first-ever
    // node startup (or migration from a pre-slice deployment).
