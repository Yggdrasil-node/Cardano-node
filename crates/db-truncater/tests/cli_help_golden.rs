//! Golden test: yggdrasil-db-truncater `--help` / `--version` outputs are
//! byte-equivalent to the upstream `db-truncater` binary.
...
fn cargo_bin() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_db-truncater"))
}
