...
pub struct MempoolSnapshot<'a> {
    entries: &'a [IndexedMempoolEntry],
}

impl<'a> MempoolSnapshot<'a> {
    ...
    pub fn mempool_lookup_tx(&self, idx: MempoolIdx) -> Option<&'a MempoolEntry> {
        self.entries
            .iter()
    ...
    pub fn mempool_lookup_tx_by_id(&self, tx_id: &TxId) -> Option<&'a MempoolEntry> {
        self.entries
            .iter()
...
impl<'a> TxSubmissionMempoolReader<'a> {
    ...
    pub fn mempool_get_snapshot(&self) -> MempoolSnapshot<'a> {
        MempoolSnapshot {
...
#[cfg(test)]
mod tests {
    use super::{Mempool, MempoolEntry, SlotNo, TxId};
