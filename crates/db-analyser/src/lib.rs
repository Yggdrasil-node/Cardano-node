//! ChainDB forensic analyser — pure-Rust port mirroring upstream Cardano.Tools.DBAnalyser.* + app/db-analyser.hs.
//!
//! ## Naming parity
//!
//! **Strict mirror:** none. Yggdrasil-side parent shell + R327
//! skeleton entry point for the `db-analyser` sister-tool crate. Per-file
...
pub fn run() -> eyre::Result<()> {
    Err(eyre::eyre!(
        "yggdrasil-db-analyser: not yet implemented (R327 skeleton); \
         see docs/operational-runs/ for the db-analyser port progress."
    ))
}
