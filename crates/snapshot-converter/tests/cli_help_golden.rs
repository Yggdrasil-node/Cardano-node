//! Golden test: yggdrasil-snapshot-converter `--help` / `--version` outputs are
//! byte-equivalent to the upstream `snapshot-converter` binary.
...
fn cargo_bin() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_snapshot-converter"))
}
