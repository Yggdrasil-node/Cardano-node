...
- [Flat encoding instances](../../.reference-haskell-cardano-node/deps/plutus/plutus-core/plutus-core/src/PlutusCore/FlatInstances.hs)
- [Plutus core tests](../../.reference-haskell-cardano-node/deps/plutus/plutus-core/plutus-core/test) — use `Evaluation`, `Flat/golden`, `Parser`, and `CostModelSafety` as upstream parity anchors for future Rust fixtures.
...
- **Integer semantics**: UPLC `Constant::Integer`, Flat integer decoding, all integer builtins, and integer ExMemory sizing use arbitrary-precision `BigInt`, matching upstream Haskell `Integer`. Do not reintroduce fixed-width `i128`/`u256` clamps for script arithmetic; only convert to bounded integers at explicit host API boundaries after range checks.
...
- **Dependencies**: `yggdrasil-crypto` (blake2b, ed25519, secp256k1), `yggdrasil-ledger` (CBOR, PlutusData), `num-bigint`/`num-integer`/`num-traits` (upstream-compatible arbitrary-precision `Integer` arithmetic), `sha2`, `sha3`, `ripemd`.

### Module Layout
- `types.rs` — `Term`, `Constant`, `Value`, `DefaultFun` (88 builtin variants), `ExBudget`, `Type`.
- `flat.rs` — Flat binary codec, bit-level reader, UPLC `Program` deserialization. `read_integer()` returns arbitrary-precision `BigInt`; `read_natural()` remains `u64`.
...
### Integration with `crates/node` crates
`crates/node/plutus-eval/src/lib.rs` implements `yggdrasil_ledger::plutus_validation::PlutusEvaluator` using the CEK machine. `CekPlutusEvaluator` decodes upstream `PlutusBinary` bytes via `decode_script_bytes` / `decode_script_bytes_allowing_remainder`, builds term-level argument applications (datum if spending, redeemer, version-aware `ScriptContext`), and evaluates with the `ExBudget` declared by the transaction. V1/V2 keep upstream's historical trailing-remainder tolerance after the first script CBOR bytestring; V3 is strict. The node-side context builder now derives `TxInfo` from the normalized ledger `TxContext`, including resolved inputs/reference inputs, structured Shelley-family TxOut addresses, withdrawals, certificates, datums, redeemers, and Conway governance data. Unsupported V3 certificate or proposal encodings now fail explicitly instead of fabricating placeholder integers. Active protocol cost-model arrays are mapped to named parameters in `crates/node/genesis/src/lib.rs`, then built with `CostModel::from_alonzo_genesis_params_with_variant()` so PV7-era V1/V2 replay uses variant A and Conway-era replay uses variant B/C as upstream. `CostModel::builtin_cost()` evaluates entries against runtime argument ExMemory sizes (with flat fallback only for unmapped builtins). The node-side Conway path maps the live 302-entry `plutusV3CostModel` array into this same named/per-builtin pipeline. `map_machine_error` at the `crates/node`/ledger boundary calls `into_ledger_error()` — operational errors collapse to opaque `PlutusScriptFailed`, `FlatDecodeError` maps to `PlutusScriptDecodeError`, and structural errors (e.g. `OutOfBudget`) preserve full detail. For replay investigations only, `YGGDRASIL_PLUTUS_TRACE_FAILURES=1` appends the raw `MachineError` to failed script diagnostics; keep the default ledger error opaque.
...
`serialiseData` delegates to `yggdrasil_ledger::PlutusData::encode_cbor`;
that encoder is consensus-visible for CEK results. Keep it aligned with
upstream `PlutusCore.Data`: non-empty lists and constructor field lists use
the Haskell-list indefinite-array shape, empty lists are `80`, bytes are
chunked at 64 bytes, and constructor tags cover compact 121-127,
extended 1280-1400, and general 102.
...
2. Close Gap BP with a captured preview V2 script/transaction fixture and per-step/per-builtin CEK cost comparison against upstream `Cek/Internal.hs::stepAndMaybeSpend`.
3. Extend Conway-array support when vendored genesis files pick up later V3+/Plomin tail parameters beyond the current 302-entry surface. **Where to edit**: the array-length pin and named-parameter mapping live in `crates/node/genesis/src/lib.rs` (`SUPPORTED_CONWAY_V3_ARRAY_LENGTHS`, `conway_v3_named_params` table, `UnsupportedConwayV3ArrayLength` / `IncompleteConwayV3Mapping` errors), not in this crate. `cost_model.rs::from_alonzo_genesis_params` consumes the already-mapped named map. A future Plomin-tail extension is a node-side change with a new fail-loud drift-pin test in this crate's per-builtin coverage tests.
