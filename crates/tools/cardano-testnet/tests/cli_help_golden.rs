//! Golden test: yggdrasil-cardano-testnet `--help` / `--version` outputs are
//! byte-equivalent to the upstream `cardano-testnet` binary.
...
fn cargo_bin() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_cardano-testnet"))
}
...
#[test]
fn version_subcommand_matches_upstream() {
    let output = Command::new(cargo_bin())
        .arg("version")
        .output()
        .expect("spawn");
