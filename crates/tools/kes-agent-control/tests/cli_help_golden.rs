//! Golden test: yggdrasil-kes-agent-control `--help` / `--version` outputs are
//! byte-equivalent to the upstream `kes-agent-control` binary.
...
fn cargo_bin() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_kes-agent-control"))
}
