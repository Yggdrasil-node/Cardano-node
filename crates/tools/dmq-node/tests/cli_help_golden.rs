//! Golden test: yggdrasil-dmq-node `--help` / `--version` outputs are
//! byte-equivalent to the upstream `dmq-node` binary.
...
fn cargo_bin() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_dmq-node"))
}
