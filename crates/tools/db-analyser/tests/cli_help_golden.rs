//! Golden test: yggdrasil-db-analyser `--help` / `--version` outputs are
//! byte-equivalent to the upstream `db-analyser` binary.
...
fn cargo_bin() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_BIN_EXE_db-analyser"))
}
