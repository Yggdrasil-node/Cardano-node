...
//! ## Naming parity
//!
//! **Strict mirror:** deps/kes-agent/kes-agent/src/Cardano/KESAgent/Protocols/Control/V1/Driver.hs.
//!
//! Mirrors the pure discriminator and read-error mapping surface of
//! upstream `Control.V1.Driver`. Raw bearer send/receive and direct
//! serialization remain deferred to the daemon/socket follow-on.

use super::control_v1_protocol::ControlMessage;
...
/// idle-state Control V1 command messages.
...
/// Control V1 command discriminator.
    ...
    fn read_error_to_control_driver_trace_matches_upstream_mapping() {
        ...
        assert_eq!(
            read_error_to_control_driver_trace(ReadResult::ReadVersionMismatch {
                expected: mk_version_identifier("Control:1.0"),
                actual: mk_version_identifier("Control:2.0"),
            }),
