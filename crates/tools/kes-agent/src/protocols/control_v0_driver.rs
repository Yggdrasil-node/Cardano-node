...
//! ## Naming parity
//!
//! **Strict mirror:** deps/kes-agent/kes-agent/src/Cardano/KESAgent/Protocols/Control/V0/Driver.hs.
//!
//! Mirrors the pure discriminator and read-error mapping surface of
//! upstream `Control.V0.Driver`. Raw bearer send/receive and direct
//! serialization remain deferred to the daemon/socket follow-on.

use super::control_v0_protocol::ControlMessage;
...
/// idle-state Control V0 command messages.
...
/// Control V0 command discriminator.
pub fn control_message_for_command(command: Command, ocert: Option<Vec<u8>>) -> ControlMessage {
    match command {
        ...
        Command::DropKeyCmd => ControlMessage::ProtocolErrorMessage,
...
mod tests {
    ...
    fn read_error_to_control_driver_trace_matches_upstream_mapping() {
        ...
        assert_eq!(
            read_error_to_control_driver_trace(ReadResult::ReadVersionMismatch {
                expected: mk_version_identifier("Control:StandardCrypto:0.5"),
                actual: mk_version_identifier("Control:1.0"),
            }),
            ControlDriverTrace::ControlDriverInvalidVersion(
                mk_version_identifier("Control:StandardCrypto:0.5"),
    ...
    fn control_message_for_command_matches_upstream_recv_message_cases() {
        ...
        assert_eq!(
            ...
            ControlMessage::ProtocolErrorMessage
        );
        assert_eq!(
