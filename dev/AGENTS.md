# Guidance for workspace development helpers under `dev/`.

This directory hosts vendored-tree refresh tooling, CI parity validators,
evidence/comparison harnesses, and operator/runbook shell helpers. The split
keeps helper ownership explicit: `dev/test/` for validators, `dev/reference/`
for the vendored Haskell snapshot/install flow, `dev/evidence/` for parity
evidence harnesses, and `dev/scripts/` for operator convenience scripts.
...
dev/
├── reference/
│   ├── setup-reference.sh             # one-shot vendored-tree fetch/install helper
│   └── install_haskell_cardano_node.sh # Linux/WSL reference binary installer
├── test/
...
│   ├── check-reference-artifacts.py   # Linux/WSL local-only reference-install validator
│   ├── check-core-evidence-harnesses.py # local preflight for evidence helper self-tests
│   ├── check-core-closeout-artifacts.py # strict final live core closeout artifact gate
│   └── filetree.py                    # checked-in filetree manifest maintenance
├── evidence/
│   ├── stage-core-closeout-artifacts.py # stages live closeout artifacts then runs final gate
│   ├── compare_tip_to_haskell.sh      # upstream Haskell tip comparison helper
│   ├── parallel_blockfetch_soak.sh    # runbook Section 6.5 BlockFetch evidence gate
│   └── compare-*                      # Gap BO/BP/R178 and sister-tool comparison helpers
├── scripts/
│   ├── check_upstream_drift.sh        # operator pin-drift report
│   ├── run-tools.sh                   # sister-tool launcher
│   └── *producer* helpers             # operator runbook convenience scripts
└── filetree/                          # accepted filetree metadata + rendered inventory
```
...
- **Mirror/docstring violation**: a new production `.rs` was added
  ...
  `## Naming parity` block. Author the docstring using the strict-mirror
  policy in root `AGENTS.md`, or rename the file to mirror an upstream `.hs`.
...
nested configuration/scripts under any `crates/node/*` package, or retired
AI-harness paths. It also fails on exact stale filesystem directories even if
...
the preset carries it), `configuration/poolMetaData.json`,
`dev/scripts/run-tools.sh`, reference/operator helpers under
`dev/{reference,evidence,scripts}/`, and `dev/scripts/yggdrasil-node.service`.
Every tracked `dev/{scripts,evidence,reference}/*.sh` file must also keep Git executable mode
...
the full `dev/` helper tree, and the Dockerfile must copy root
`configuration/` plus the required helper scripts from `dev/scripts/`. Docker must also pin
...
`dev/scripts/yggdrasil-node.service` unit must set
...
instructions must use `crates/node/cardano-node/`, root `configuration/`, and
`dev/{scripts,evidence,reference,test}/`.

Run `python3 dev/test/check-stale-placement.py --self-test` before the scan
...
Run `python3 dev/test/check-doc-status-headers.py --self-test` before the live
...
Run after `bash dev/reference/setup-reference.sh --force` to confirm the
...
```
python3 dev/test/check-core-closeout-artifacts.py
```
...
Gap BO closeout runs must pass `--require-haskell --require-equal`; the helper
...
`dev/test/check-parity-matrix.py`, and prose mentions in root `AGENTS.md`.
...
- Document a new validator in root `AGENTS.md` + the enclosing crate
  `AGENTS.md` if its scope is crate-local.
- Keep CI validators and operator harnesses named by purpose; do not add
  session-local helper surfaces here.
