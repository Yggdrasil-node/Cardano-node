...
layout:
  ...
  - development scripts/tools: dev/{scripts,test,evidence,reference}/
...
STALE_DIRECTORIES = {
    ...
    "retired Claude harness directory": ".claude",
    "retired Claude guidance file": "CLAUDE.md",
}
...
BASE_REQUIRED_CURRENT_PLACEMENTS = {
    ...
    "dev sister-tool launcher": "dev/scripts/run-tools.sh",
    "dev reference setup helper": "dev/reference/setup-reference.sh",
    "dev Haskell install helper": "dev/reference/install_haskell_cardano_node.sh",
    "dev Haskell tip comparison harness": "dev/evidence/compare_tip_to_haskell.sh",
    "dev parallel blockfetch soak harness": "dev/evidence/parallel_blockfetch_soak.sh",
    "dev preview producer harness": "dev/scripts/preview_producer_harness.sh",
    "dev systemd unit template": "dev/scripts/yggdrasil-node.service",
...
REQUIRED_TEXT_SNIPPETS = (
    ...
    (
        ".github/workflows/release.yml",
        "release workflow stages dev tooling bundle",
        'cp -r dev "$dir/dev"',
    ),
    (
    ...
    (
        ".github/workflows/repro-check.yml",
        "repro workflow stages dev tooling bundle",
        'cp -r dev "$dir/dev"',
    ),
    (
        "Dockerfile",
    ...
    (
        ...
        "COPY --from=builder /src/dev/scripts/check_upstream_drift.sh",
    ),
    (
        "Dockerfile",
        "Docker image copies root restart-resilience helper",
        "COPY --from=builder /src/dev/scripts/restart_resilience.sh",
    ),
    (
    ...
    (
        "dev/scripts/install_from_release.sh",
        "release installer requires bundled dev tooling",
        '[ -d "${extracted_dir}/dev" ] || err "extracted archive missing dev/ bundle"',
    ),
    (
        "dev/scripts/install_from_release.sh",
    ...
    (
        "dev/scripts/install_from_release.sh",
        "release installer installs dev tooling bundle",
        'run_install cp -R "${extracted_dir}/dev" "${share_target}/"',
    ),
    (
...
STALE_PATTERNS = {
    ...
    "old yggdrasil-node shorthand path": re.compile(
        ...
        r"Cargo\.toml|configuration|scripts|dev[/\\]scripts|tests)(?=$|[/\\])"
    ),
    ...
    "scripts nested under cardano-node crate": re.compile(
        r"crates[/\\]node[/\\]cardano-node[/\\](?:dev[/\\])?scripts"
    ),
    ...
    "operator scripts nested under node crate": re.compile(
        r"crates[/\\]node[/\\][^/\\]+[/\\](?:dev[/\\])?scripts(?=$|[/\\])"
    ),
    "developer diagnostics nested under cardano-node crate": re.compile(
        r"crates[/\\]node[/\\]cardano-node[/\\]src[/\\]bin(?=$|[/\\])"
    ),
    "retired Claude harness path": re.compile(r"(?<![A-Za-z0-9_.-])\.claude(?=$|[/\\])"),
    "retired Claude guidance file": re.compile(r"(?<![A-Za-z0-9_.-])CLAUDE\.md(?![A-Za-z0-9_.-])"),
...
SELF_TEST_STALE_CASES = (
    ...
    ("yggdrasil-node/dev/scripts/run-tools.sh", "old yggdrasil-node shorthand path"),
    ...
    ("node/dev/scripts/run-tools.sh", "scripts nested under node"),
    (
        "crates/node/cardano-node/configuration/mainnet/config.json",
        "config nested under cardano-node crate",
    ),
    ("crates/node/cardano-node/dev/scripts/run-tools.sh", "scripts nested under cardano-node crate"),
    (
        "crates/node/runtime/configuration/mainnet/config.json",
        "operator configuration nested under node crate",
    ),
    ("crates/node/runtime/dev/scripts/run-tools.sh", "operator scripts nested under node crate"),
    (
        "crates/node/cardano-node/src/bin/dump_block.rs",
        "developer diagnostics nested under cardano-node crate",
    ),
    (".claude/skills/cardano-node/SKILL.md", "retired Claude harness path"),
    (".claude/settings.json", "retired Claude harness path"),
    ("CLAUDE.md", "retired Claude guidance file"),
    (
...
def dev_shell_script_mode_failures(
    root: Path = ROOT,
    *,
    stage_output: bytes | None = None,
) -> list[str]:
    """Require tracked dev shell helpers to retain executable file mode."""

    if stage_output is None:
        try:
            result = subprocess.run(
                ["git", "ls-files", "--stage", "-z", "--", "dev/scripts/*.sh", "dev/evidence/*.sh", "dev/reference/*.sh"],
                cwd=root,
                check=True,
                stdout=subprocess.PIPE,
            )
        except (OSError, subprocess.CalledProcessError) as exc:
            return [f"dev/**/*.sh:0: failed to inspect executable modes: {exc}"]
    ...
    for entry in stage_output.split(b"\0"):
        ...
        if not (
            normalized_path.startswith("dev/scripts/")
            or normalized_path.startswith("dev/evidence/")
            or normalized_path.startswith("dev/reference/")
        ) or not normalized_path.endswith(".sh"):
            continue
        tracked_shell_scripts += 1
        mode = metadata.split(" ", 1)[0]
        if mode != "100755":
            failures.append(
                f"{normalized_path}:0: dev shell script must be "
                f"tracked executable (100755); got {mode}"
            )

    if tracked_shell_scripts == 0:
        failures.append("dev/**/*.sh:0: no tracked dev shell scripts found")
...
def main() -> int:
    ...
    failures.extend(dev_shell_script_mode_failures())
...
def self_test() -> int:
    ...
    if should_scan("dev/test/check-stale-placement.py"):
    ...
    executable_mode_failures = dev_shell_script_mode_failures(
        stage_output=(
            f"100644 {'0' * 40} 0\tdev/scripts/run-tools.sh\0"
            f"100755 {'0' * 40} 0\tdev/reference/setup-reference.sh\0"
            f"100644 {'0' * 40} 0\tdev/scripts/yggdrasil-node.service\0"
            f"100644 {'0' * 40} 0\tdocs/example.sh\0"
        ).encode("utf-8")
    )
    if not any("dev/scripts/run-tools.sh" in failure for failure in executable_mode_failures):
        failures.append("non-executable dev shell scripts should be rejected")
    if any("dev/reference/setup-reference.sh" in failure for failure in executable_mode_failures):
        failures.append("executable dev shell scripts should be accepted")
    if any("dev/scripts/yggdrasil-node.service" in failure for failure in executable_mode_failures):
        failures.append("non-shell dev script artifacts should not require executable mode")
    if any("docs/example.sh" in failure for failure in executable_mode_failures):
        failures.append("non-dev shell scripts should not be handled by placement guard")

    if dev_shell_script_mode_failures(
        stage_output=(f"100755 {'0' * 40} 0\tdev/scripts/run-tools.sh\0").encode(
            "utf-8"
        )
    ):
        failures.append("all-executable dev shell script stage output should be accepted")

    if not dev_shell_script_mode_failures(stage_output=b""):
        failures.append("missing tracked dev shell scripts should be rejected")
