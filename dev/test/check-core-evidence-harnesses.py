...
def require_wsl_or_linux() -> None:
    if sys.platform == "win32":
        raise SystemExit(
            ...
            "use `wsl -e bash -lc \"python3 dev/test/check-core-evidence-harnesses.py\"`"
        )
...
def build_cases() -> list[Case]:
    ...
    cases = [
        Case(
            "gap-bo-tpraos-vrf",
            [python, "dev/evidence/compare-gap-bo-tpraos-vrf.py", "--self-test"],
        ),
        Case(
            "gap-bp-script-context",
            [python, "dev/evidence/compare-gap-bp-script-context.py", "--self-test"],
        ),
        Case(
            "gap-bp-cek-flushes",
            [python, "dev/evidence/compare-gap-bp-cek-flushes.py", "--self-test"],
        ),
        Case(
            "gap-bp-builtin-costs",
            [python, "dev/evidence/compare-gap-bp-builtin-costs.py", "--self-test"],
        ),
        Case(
            "gap-bp-traces",
            [python, "dev/evidence/compare-gap-bp-traces.py", "--self-test"],
        ),
        Case(
            "r178-conway-lsq",
            [python, "dev/evidence/compare-conway-lsq.py", "--self-test"],
        ),
        Case(
            "core-closeout-staging",
            [python, "dev/evidence/stage-core-closeout-artifacts.py", "--self-test"],
        ),
    ]
    ...
    if bash is None:
        cases.append(
            Case(
                "tip-comparison",
                ["bash", "dev/evidence/compare_tip_to_haskell.sh", "--self-test"],
            )
        )
        cases.append(
            Case(
        ...
        cases.append(
            Case(
                "blockfetch-soak",
                [bash, "dev/evidence/parallel_blockfetch_soak.sh", "--self-test"],
            )
        )
    return cases
