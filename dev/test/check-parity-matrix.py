...
The script enforces:
  ...
  - The local reference snapshot carries the generated REFERENCE_TAG marker
    from dev/reference/setup-reference.sh and it matches the pinned release.
...
# The living plan is in docs/COMPLETION_ROADMAP.md; each tool's
# `next_milestone` advances through its mini-arc
...
def validate_reference_marker(local_root: str) -> None:
    ...
    if not marker.is_file():
        fail(
            ...
            "run dev/reference/setup-reference.sh"
        )
    try:
...
def main() -> int:
    ...
    if not (ROOT / local_root).is_dir():
        fail(
            ...
            f"run dev/reference/setup-reference.sh: {local_root}"
        )
