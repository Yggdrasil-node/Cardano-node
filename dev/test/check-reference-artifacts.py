...
def installed_node_version() -> str:
    ...
    if not binary.is_file():
        fail(
            ...
            f"`bash dev/reference/setup-reference.sh` to populate the install tree."
        )
    try:
        proc = subprocess.run(
            [str(binary), "--version"],
...
def main() -> None:
    ...
    if not INSTALL_ROOT.is_dir():
        fail(
            ...
            f"`bash dev/reference/setup-reference.sh` to populate it."
        )
    if sys.platform != "linux":
        fail(
            "this gate must run under Linux/WSL because "
            ".reference-haskell-cardano-node/install/bin contains Linux "
            "executables. Use `bash dev/reference/setup-reference.sh --sources-only` "
    ...
    if installed != tag:
        fail(
            ...
            f"`bash dev/reference/setup-reference.sh --force` to rebase to {tag}."
        )
