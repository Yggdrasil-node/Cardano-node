...
# `crates/tools/{bech32, cardano-submit-api, cardano-testnet, cardano-tracer,
...
# entries under `crates/tools/<tool>/` AND the per-tool `[[bin]] name = "<tool>"`
# declarations. Keep in lockstep with `crates/node/config/src/upstream_pins.rs`
# `UPSTREAM_PINS` for the sister-tool subset.
TOOLS=(
    bech32
    cardano-submit-api
    cardano-testnet
...
print_list() {
    ...
    for tool in "${TOOLS[@]}"; do
        local crate_dir="$ROOT_DIR/crates/tools/$tool"
        if [[ -d "$crate_dir" ]]; then
            echo "  $tool"
        else
