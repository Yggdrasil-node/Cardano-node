#!/usr/bin/env bash
# install_from_release.sh — download and install a Yggdrasil release bundle.
#
# Usage:
...
# SHA256 against the published SHA256SUMS.txt, installs the binary to
# /usr/local/bin (or the prefix given as the second argument), and installs the
# bundled configuration/scripts under <prefix>/share/yggdrasil/.
#
...
share_target="${PREFIX}/share/yggdrasil"

info "installing binary to ${bin_target} (may require sudo)..."
run_install install -d "${PREFIX}/bin" "${share_target}"
run_install install -m 0755 "${extracted_dir}/${BINARY_NAME}" "${bin_target}"

info "installing configuration and dev tooling to ${share_target}..."
...
ok "installed operator artifacts: ${share_target}/configuration and ${share_target}/dev"
echo
echo "Next steps:"
...
echo "  Presets resolve from ${share_target}/configuration; set YGGDRASIL_CONFIG_ROOT to override."
echo "  Optional: inspect installed presets under ${share_target}/configuration"
echo
